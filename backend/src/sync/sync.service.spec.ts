...
function createService(prisma: unknown, operations: Record<string, jest.Mock>, attendance: Record<string, jest.Mock> = {}) {
  const members = { create: operations.createMember, findByCi: operations.findMemberByCi, update: operations.updateMember, delete: operations.deleteMember, list: operations.listMembers };
  const memberships = { createMemberWithMembership: operations.createMemberWithMembership, create: operations.createMembership, update: operations.updateMembership, renew: operations.renewMembership, delete: operations.deleteMembership };
  const payments = { apply: operations.applyPayment, list: operations.listPayments };
  const plans = { create: operations.createPlan, findByName: operations.findPlanByName, update: operations.updatePlan, delete: operations.deletePlan, list: operations.listPlans };
  const dashboards = { adminDashboard: operations.adminDashboard };
  return new SyncService(prisma as never, attendance as never, members as never, memberships as never, payments as never, plans as never, dashboards as never);
}
...
describe('SyncService', () => {
  ...
  it('devuelve el recibo anterior y no repite una operación aplicada', async () => {
    ...
    const service = createService(prisma, mini);
  ...
  it('fusiona por CI y remapea referencias posteriores del mismo lote', async () => {
    ...
    const service = createService(prisma, mini, attendance);
    const assign = {
