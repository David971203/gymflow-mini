...
import { CheckInAttendanceDto, CheckOutAttendanceDto } from '../attendance/attendance.dto';
import { AttendanceService } from '../attendance/attendance.service';
import { CreateMemberDto, UpdateMemberDto } from '../members/members.dto';
import { CreateMemberWithMembershipDto, CreateMembershipDto, RenewMembershipDto, UpdateGymMembershipDto } from '../memberships/memberships.dto';
import { ApplyPaymentDto } from '../payments/payments.dto';
import { CreatePlanDto, UpdatePlanDto } from '../plans/plans.dto';
import { SyncOperationDto, SyncPushDto } from './sync.dto';
...
export class SyncService {
  ...
  async snapshot(user: AuthUser) {
    const [dashboard, members, plans, payments, attendances] = await Promise.all([
      this.dashboards.adminDashboard(user),
      this.members.list(user),
      this.plans.list(user),
      this.payments.list(user),
  ...
  private async apply(operation: SyncOperationDto, user: AuthUser): Promise<Pick<SyncResult, 'status' | 'result'>> {
    ...
    switch (operation.type) {
      case 'MEMBER_CREATE': {
        ...
        try {
          const member = await this.members.create(dto, user);
          return { status: 'APPLIED', result: { id: member.id } };
        } catch (error) {
          if (!(error instanceof ConflictException)) throw error;
          const member = await this.members.findByCi(dto.ci, user);
      ...
      case 'MEMBER_CREATE_WITH_MEMBERSHIP': {
        ...
        const result = await this.memberships.createMemberWithMembership(dto, user, true);
      ...
      case 'MEMBER_UPDATE': {
        ...
        const member = await this.members.update(operation.entityId, dto, user);
        return { status: 'APPLIED', result: { id: member.id } };
      }
      case 'MEMBER_DELETE': {
        const result = await this.members.delete(operation.entityId, user);
        return { status: 'APPLIED', result };
      }
      case 'PLAN_CREATE': {
        const dto = await this.payload(CreatePlanDto, { ...operation.payload, clientId: operation.entityId });
        try {
          const plan = await this.plans.create(dto, user);
          return { status: 'APPLIED', result: { id: plan.id } };
        } catch (error) {
          if (!(error instanceof ConflictException)) throw error;
          const plan = await this.plans.findByName(dto.name, user);
          if (!plan) throw error;
      ...
      case 'PLAN_UPDATE': {
        ...
        const plan = await this.plans.update(operation.entityId, dto, user);
        return { status: 'APPLIED', result: { id: plan.id } };
      }
      case 'PLAN_DELETE': {
        const result = await this.plans.delete(operation.entityId, user);
        return { status: 'APPLIED', result };
      }
      case 'MEMBERSHIP_ASSIGN': {
        ...
        const membership = await this.memberships.create(dto, user);
      ...
      case 'MEMBERSHIP_UPDATE': {
        ...
        const membership = await this.memberships.update(operation.entityId, dto, user);
      ...
      case 'MEMBERSHIP_RENEW': {
        ...
        const membership = await this.memberships.renew(operation.entityId, dto, user);
        return { status: 'APPLIED', result: { id: membership.id, paymentId: membership.payment?.id ?? null } };
      }
      case 'MEMBERSHIP_DELETE': {
        const result = await this.memberships.delete(operation.entityId, user);
        return { status: 'APPLIED', result };
      }
      case 'PAYMENT_APPLY': {
        ...
        const payment = await this.payments.apply(operation.entityId, dto, user);
