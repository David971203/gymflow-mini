...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section pricing" id="planes"><div className="section-heading"><div><span className="eyebrow">PLANES SIMPLES Y TRANSPARENTES</span><h2>Elige el ritmo que mejor funciona para ti.</h2></div><p>Descarga la app, crea tu cuenta y elige el plan dentro. Solo usamos WhatsApp para coordinar los pagos P2P.</p></div><div className="pricing-grid">{plans.map(plan=><article key={plan.id} className={plan.featured ? "featured" : ""}>{plan.badge&&<span className="popular">{plan.badge}</span>}<span className="plan-tag">{plan.tag}</span><h3>{plan.name}</h3><div className="price"><strong>{plan.price}</strong><small>{plan.suffix}</small></div><p>{plan.copy}</p><hr/><div className="plan-features">{plan.features.map(feature=><span key={feature}><Check/>{feature}</span>)}</div><a className="plan-cta" href='https://expo.dev/artifacts/eas/Hcbxq7cMlrXoH8Gx-w-q1YYbbx_QaN9tXRwZYyxNUcA.apk'><Smartphone/>{plan.cta}</a></article>)}</div></section>
