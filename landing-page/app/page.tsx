import {
  ...
  Dumbbell, Facebook, Gauge, Heart, Instagram, Layers3, MapPin, Menu,
  MessageCircle, RefreshCw, ShieldCheck, Smartphone, Users, Wifi,
  TrendingUp,
} from "lucide-react";
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section pricing" id="planes"><div className="section-heading"><div><span className="eyebrow">PLANES SIMPLES Y TRANSPARENTES</span><h2>Elige el ritmo que mejor funciona para ti.</h2></div><p>Prueba toda la experiencia durante 7 días. Cuando estés listo, continúa sin perder tus datos.</p></div><div className="pricing-grid">{plans.map(plan=><article key={plan.id} className={plan.featured ? "featured" : ""}>{plan.badge&&<span className="popular">{plan.badge}</span>}<span className="plan-tag">{plan.tag}</span><h3>{plan.name}</h3><div className="price"><strong>{plan.price}</strong><small>{plan.suffix}</small></div><p>{plan.copy}</p><hr/><div className="plan-features">{plan.features.map(feature=><span key={feature}><Check/>{feature}</span>)}</div><a className="plan-cta" href={whatsappUrl(plan.name)} target="_blank" rel="noreferrer">{plan.id !== "trial" && <MessageCircle/>}{plan.cta}</a></article>)}</div><p className="whatsapp-note">Los botones de plan abrirán WhatsApp con el plan elegido ya incluido en el mensaje.</p></section>
