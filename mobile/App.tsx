...
function publishToast(message: string, tone: ToastTone) {
  ...
  const subscriber = tone === 'success' ? subscribers[subscribers.length - 1] : subscribers[0];
...
function ToastHost({ active = true, priority = 0 }: { active?: boolean; priority?: number }) {
  ...
  useEffect(() => {
    ...
    const show = (nextToast: ToastMessage) => {
      ...
      setToast(nextToast);
      hideTimer.current = setTimeout(() => setToast(null), nextToast.tone === 'error' ? 4500 : 3200);
    };
  ...
  return <View pointerEvents="box-none" style={styles.errorToastLayer}>
    <View accessibilityRole="alert" accessibilityLiveRegion={success ? 'polite' : 'assertive'} style={[styles.errorToast, success && styles.successToast]}>
      <Ionicons name={success ? 'checkmark-circle' : 'alert-circle'} size={23} color="#fff" />
      <View style={styles.errorToastBody}><Text style={styles.errorToastTitle}>{success ? 'Listo' : 'No se pudo completar'}</Text><Text style={styles.errorToastMessage}>{toast.message}</Text></View>
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.loginKeyboardAvoiding}>
      <ScrollView ref={scrollRef} contentContainerStyle={styles.loginScroll} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.loginJoin}>
          <Text style={styles.loginJoinText}>¿No tienes una cuenta?</Text>
...
function PasswordRecovery({ onBack }: { onBack: () => void }) {
  ...
  const scrollRef=useRef<ScrollView>(null); const reveal=useRevealFocusedInput(scrollRef,28);
  ...
  return <SafeAreaView style={styles.loginPage} edges={['top','right','bottom','left']}><ExpoStatusBar style="light" translucent backgroundColor="transparent"/><KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={styles.loginKeyboardAvoiding}><ScrollView ref={scrollRef} contentContainerStyle={styles.registerScroll} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==='ios'?'interactive':'on-drag'} showsVerticalScrollIndicator={false}><KeyboardScrollContext.Provider value={reveal}>
...
function Register({ onRegistered, onBack }: { onRegistered: (user: User) => void; onBack: () => void }) {
  ...
  return <SafeAreaView style={styles.loginPage} edges={['top','right','bottom','left']}><ExpoStatusBar style="light" translucent backgroundColor="transparent"/><KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={styles.loginKeyboardAvoiding}><ScrollView ref={scrollRef} contentContainerStyle={styles.registerScroll} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==='ios'?'interactive':'on-drag'} showsVerticalScrollIndicator={false}><KeyboardScrollContext.Provider value={reveal}>
    <Pressable accessibilityRole="button" accessibilityLabel="Volver al inicio de sesión" onPress={onBack} style={styles.authBack}><Ionicons name="arrow-back" size={20} color={palette.white}/><Text style={styles.authBackText}>Volver</Text></Pressable>
    <Text style={styles.loginBrand}>GymFlow <Text style={styles.mini}>MINI</Text></Text><Text style={styles.registerTitle}>Crea tu cuenta</Text><Text style={styles.loginCopy}>Cuéntanos lo esencial de ti y de tu gimnasio.</Text>
    <View style={styles.loginCard}><Field label="Tu nombre y apellidos" value={ownerName} onChangeText={setOwnerName} autoCapitalize="words"/><Field label="Nombre del gimnasio" value={gymName} onChangeText={setGymName} autoCapitalize="words"/><Field label="Provincia (opcional)" value={province} onChangeText={setProvince} autoCapitalize="words"/><Field label="Teléfono móvil" value={phone} onChangeText={setPhone} keyboardType="phone-pad" placeholder="5XXXXXXX"/><Field label="Correo" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none"/><Field label="Contraseña" value={password} onChangeText={setPassword} secureTextEntry autoCapitalize="none"/><Field label="Repite la contraseña" value={confirmation} onChangeText={setConfirmation} secureTextEntry autoCapitalize="none" returnKeyType="done" onSubmitEditing={()=>void submit()}/><Text style={styles.registrationNote}>La prueba gratuita es única por teléfono y dispositivo.</Text><PrimaryButton label={loading?'Creando cuenta…':'Crear cuenta'} onPress={submit} disabled={loading||!ownerName.trim()||!gymName.trim()||!phone.trim()||!email.trim()||password.length<8||!confirmation}/></View>
  </KeyboardScrollContext.Provider></ScrollView></KeyboardAvoidingView></SafeAreaView>;
}
...
function SubscriptionSetup({ user, onUserChange, onLogout }: { user: User; onUserChange: (user: User) => void; onLogout: () => void }) {
  ...
  const refresh=async()=>{setLoading(request?.plan??'MONTHLY');try{const updated=await api.refreshProfile();onUserChange(updated);if(updated.gym.subscriptionPlan)showSuccess('Tu plan ya está activo.');else showSuccess('La solicitud continúa pendiente.');}catch(error){showError(error);}finally{setLoading(null);}};
...
function AdminApp({ user, dark, themePreference, onThemeChange, onLogout }: { user: User; dark: boolean; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void }) {
  ...
  return <SafeAreaView style={styles.app} edges={['top','right','bottom','left']}><StatusBar translucent backgroundColor="transparent" barStyle={dark ? "light-content" : "dark-content"} />
    <View style={styles.topbar}>
      <View style={styles.topbarTitle}><Text numberOfLines={1} ellipsizeMode="tail" maxFontSizeMultiplier={1.3} style={styles.kicker}>{currentUser.gym.name.toUpperCase()}</Text><Text maxFontSizeMultiplier={1.3} style={styles.screenTitle}>{tabs.find((item) => item.id === tab)?.label}</Text></View>
    </View>
    {(!keyboardOpen || (tab !== 'MIEMBROS' && tab !== 'PLANES')) && <SyncBar state={syncState} onPress={() => { void refreshProfile(); syncState.rejected > 0 ? setIssuesOpen(true) : void syncNow(scope); }} />}
    <View style={styles.content}>
      ...
      {tab === 'CUENTA' && <AccountScreen user={currentUser} onUserChange={setCurrentUser} themePreference={themePreference} onThemeChange={onThemeChange} onLogout={onLogout} />}
    </View>
...
function AccountScreen({ user, onUserChange, themePreference, onThemeChange, onLogout }: { user: User; onUserChange: (user: User) => void; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void }) {
  ...
  const [passwordOpen,setPasswordOpen]=useState(false); const [currentPassword,setCurrentPassword]=useState(''); const [newPassword,setNewPassword]=useState(''); const [passwordConfirmation,setPasswordConfirmation]=useState(''); const [savingPassword,setSavingPassword]=useState(false);
  ...
  const actions: Array<{label:string;message:string;plan:Exclude<GymSubscriptionPlan,'TRIAL'>}> = plan==='TRIAL'
    ? [
        { label:'Adquirir plan mensual', plan:'MONTHLY', message:`Hola, quiero adquirir el plan mensual para el gimnasio ${user.gym.name}. La prueba gratuita vence el ${expiry}.` },
        { label:'Adquirir plan anual', plan:'ANNUAL', message:`Hola, quiero adquirir el plan anual para el gimnasio ${user.gym.name}. La prueba gratuita vence el ${expiry}.` },
      ]
    : plan==='MONTHLY'
      ? [
          { label:'Renovar plan mensual', plan:'MONTHLY', message:`Hola, quiero renovar el plan mensual del gimnasio ${user.gym.name}, que vence el ${expiry}.` },
          { label:'Cambiar a plan anual', plan:'ANNUAL', message:`Hola, quiero cambiar el gimnasio ${user.gym.name} del plan mensual al plan anual cuando termine el período actual el ${expiry}.` },
        ]
      : [
          { label:'Renovar plan anual', plan:'ANNUAL', message:`Hola, quiero renovar el plan anual del gimnasio ${user.gym.name}, que vence el ${expiry}.` },
          { label:'Cambiar a plan mensual', plan:'MONTHLY', message:`Hola, quiero cambiar el gimnasio ${user.gym.name} del plan anual al plan mensual cuando termine el período actual el ${expiry}.` },
        ];
  const requestWhatsAppMessage=(current:User,prefix?:string)=>{const request=current.subscriptionRequest;if(!request)return prefix??'';const requestedPlan=request.plan==='MONTHLY'?'mensual':'anual';return `${prefix??`Hola, soy ${current.name}. Quiero gestionar el plan ${requestedPlan} para ${current.gym.name}.`} Código de solicitud: ${request.code}. Teléfono: ${current.phone??'no indicado'}. Correo: ${current.email}.`;};
  const runSubscriptionAction=async(action:(typeof actions)[number])=>{setRequestingPlan(action.plan);try{const updated=await api.selectSubscription(action.plan);onUserChange(updated);setChangingRequest(false);openWhatsApp(requestWhatsAppMessage(updated,action.message));}catch(error){showError(error);}finally{setRequestingPlan(null);}};
  const refreshRequest=async()=>{setRequestingPlan(user.subscriptionRequest?.plan??'MONTHLY');try{const updated=await api.refreshProfile();onUserChange(updated);setChangingRequest(false);showSuccess(updated.subscriptionRequest?'La solicitud continúa pendiente.':'La operación fue aprobada y tu plan está actualizado.');}catch(error){showError(error);}finally{setRequestingPlan(null);}};
  ...
  return <><ScrollView contentContainerStyle={styles.accountScroll}>
    ...
    <View style={[styles.subscriptionCard,(expired||verificationRequired)&&styles.subscriptionCardExpired]}>
      ...
      {user.subscriptionRequest&&!changingRequest?<View style={styles.accountPendingRequest}><View style={styles.accountRequestCode}><Text style={styles.accountRequestLabel}>SOLICITUD PENDIENTE</Text><Text selectable style={styles.accountRequestValue}>{user.subscriptionRequest.code}</Text></View><Text style={styles.accountRequestedPlan}>{user.subscriptionRequest.plan==='MONTHLY'?'Plan mensual · 5 000 CUP':'Plan anual · 50 000 CUP'}</Text><Pressable disabled={!!requestingPlan} accessibilityRole="link" onPress={()=>openWhatsApp(requestWhatsAppMessage(user))} style={({pressed})=>[styles.whatsappButton,styles.whatsappButtonInGroup,pressed&&styles.tabPressed]}><Ionicons name="logo-whatsapp" size={19} color={palette.white}/><Text style={styles.whatsappButtonText}>Continuar por WhatsApp</Text></Pressable><PrimaryButton label={requestingPlan?'Comprobando…':'Comprobar activación'} onPress={refreshRequest} disabled={!!requestingPlan}/><Pressable disabled={!!requestingPlan} onPress={()=>setChangingRequest(true)} style={styles.changePlanButton}><Text style={styles.changePlanText}>Cambiar la solicitud</Text></Pressable></View>:<View style={styles.subscriptionActionGroup}>{actions.map((action,index)=><Pressable key={action.label} disabled={!!requestingPlan} accessibilityRole="link" accessibilityLabel={`${action.label} por WhatsApp`} onPress={()=>void runSubscriptionAction(action)} style={({pressed}) => [styles.whatsappButton,styles.whatsappButtonInGroup,index>0&&styles.whatsappButtonSecondary,!!requestingPlan&&styles.disabled,pressed&&styles.tabPressed]}>{requestingPlan===action.plan?<ActivityIndicator color={index>0?palette.action:palette.white}/>:<Ionicons name="logo-whatsapp" size={19} color={index>0?palette.action:palette.white}/>}<Text style={[styles.whatsappButtonText,index>0&&styles.whatsappButtonTextSecondary]}>{action.label}</Text></Pressable>)}</View>}
    </View>
  ...
  </ScrollView><Sheet open={passwordOpen} title="Cambiar contraseña" onClose={()=>setPasswordOpen(false)} footer={<PrimaryButton label={savingPassword?'Guardando…':'Guardar contraseña'} onPress={()=>void savePassword()} disabled={savingPassword||currentPassword.length<8||newPassword.length<8||!passwordConfirmation}/>}><Text style={styles.sheetCopy}>Necesitas conexión. Después del cambio, las demás sesiones dejarán de ser válidas cuando intenten conectarse.</Text><Field label="Contraseña actual" value={currentPassword} onChangeText={setCurrentPassword} secureTextEntry autoCapitalize="none"/><Field label="Nueva contraseña" value={newPassword} onChangeText={setNewPassword} secureTextEntry autoCapitalize="none"/><Field label="Confirma la nueva contraseña" value={passwordConfirmation} onChangeText={setPasswordConfirmation} secureTextEntry autoCapitalize="none" returnKeyType="done" onSubmitEditing={()=>void savePassword()}/></Sheet></>;
}
...
const baseStyles = StyleSheet.create(withReadableType({
  ...
  errorToastLayer:{...StyleSheet.absoluteFillObject,zIndex:1000,elevation:30},errorToast:{position:'absolute',top:Platform.OS === 'android' ? (StatusBar.currentHeight ?? 24) + 10 : 54,left:14,right:14,minHeight:72,padding:13,flexDirection:'row',alignItems:'flex-start',gap:10,borderWidth:1,borderColor:'#f8b4ad',borderRadius:15,backgroundColor:'#b42318',shadowColor:'#7a1710',shadowOffset:{width:0,height:6},shadowOpacity:.3,shadowRadius:12,elevation:24},successToast:{borderColor:'#7cc89f',backgroundColor:'#167247',shadowColor:'#0b4a2d'},errorToastBody:{flex:1},errorToastTitle:{color:'#fff',fontSize:13,fontWeight:'900'},errorToastMessage:{marginTop:3,color:'#fff',fontSize:12,lineHeight:17,fontWeight:'600'},errorToastClose:{width:44,height:44,marginTop:-7,marginRight:-7,alignItems:'center',justifyContent:'center',borderRadius:12,backgroundColor:'#ffffff1f'},
  ...
  subscriptionActionCopy:{marginTop:14,color:palette.secondary,fontSize:10,lineHeight:15,fontWeight:'700'},subscriptionActionGroup:{marginTop:13,gap:8},whatsappButtonInGroup:{marginTop:0},whatsappButtonSecondary:{borderWidth:1,borderColor:'#b9d8c5',backgroundColor:'#f7fbf8'},whatsappButtonTextSecondary:{color:palette.action},
