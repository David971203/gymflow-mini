...
function AccountScreen({ user, onUserChange, themePreference, onThemeChange, onLogout, passwordOpen, onPasswordOpenChange }: { user: User; onUserChange: (user: User) => void; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void; passwordOpen: boolean; onPasswordOpenChange: (open: boolean) => void }) {
  ...
  return <><ScrollView contentContainerStyle={styles.accountScroll}>
    ...
    <View style={[styles.subscriptionCard,(expired||verificationRequired)&&styles.subscriptionCardExpired]}>
      ...
      <Text style={[styles.subscriptionActionCopy,(expired||verificationRequired)&&styles.subscriptionExpiredCopy]}>{user.role === 'ADMIN' ? actionCopy : 'Un administrador gestiona la renovación y los cambios del plan.'}</Text>
...
function PlansScreen({ scope, revision, readOnly }: ScreenProps & { readOnly: boolean }) {
  ...
  </ScrollView>{!readOnly ? <PlanActions plan={selected} onClose={() => setSelected(null)} onEdit={() => { void onlineAction(() => { if (selected) setEditing(selected); setSelected(null); }); }} onReactivate={() => { if (selected) reactivate(selected); }} onDelete={() => { if (selected) confirmDelete(selected); }}/> : null}</>;
}
