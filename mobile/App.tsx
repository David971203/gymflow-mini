...
function Sheet({ open, title, onClose, children }: { open: boolean; title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  if (open) { displayedTitle.current = title; displayedChildren.current = children; }
  useEffect(() => {
  ...
  return <Modal visible={mounted} transparent animationType="none" hardwareAccelerated statusBarTranslucent onRequestClose={onClose}>
    <View style={styles.modalRoot}>
      ...
      <Animated.View style={[styles.sheet, { opacity: progress, transform: [{ translateY: progress.interpolate({ inputRange:[0,1], outputRange:[52,0] }) }, { scale: progress.interpolate({ inputRange:[0,1], outputRange:[.985,1] }) }] }]}>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>{displayedChildren.current}</ScrollView>
      </Animated.View>
    </View>
  </Modal>;
}
