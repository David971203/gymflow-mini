...
import { effectiveMembershipStatus, isMembershipDayBefore, isMembershipDayOnOrBefore, membershipRemainingDays } from './src/membershipDates';
...
function SubscriptionSetup({ user, onUserChange, onLogout }: { user: User; onUserChange: (user: User) => void; onLogout: () => void }) {
  ...
  const choose=async(plan:GymSubscriptionPlan)=>{setLoading(plan);try{const updated=await api.selectSubscription(plan);onUserChange(updated);if(plan==='TRIAL')openWhatsApp(updated);}catch(error){showError(error);}finally{setLoading(null);}};
  const resendTrial=async()=>{setLoading('TRIAL');try{const updated=await api.selectSubscription('TRIAL');onUserChange(updated);openWhatsApp(updated);}catch(error){showError(error);}finally{setLoading(null);}};
...
function AdminApp({ user, dark, themePreference, onThemeChange, onLogout }: { user: User; dark: boolean; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void }) {
  ...
  const visibleTabs = currentUser.role === 'ADMIN' ? tabs : tabs.filter(item => item.id !== 'ESTADISTICAS');
  ...
  const currentScreenTitle = tab === 'ASISTENCIA' ? 'Asistencia' : tab === 'CUENTA' ? 'Cuenta' : visibleTabs.find((item) => item.id === tab)?.title ?? visibleTabs.find((item) => item.id === tab)?.label;
...
function AccountScreen({ user, onUserChange, themePreference, onThemeChange, onLogout, passwordOpen, onPasswordOpenChange }: { user: User; onUserChange: (user: User) => void; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void; passwordOpen: boolean; onPasswordOpenChange: (open: boolean) => void }) {
  ...
  const actionCopy = verificationRequired
        ...
        : expiringToday
          ...
          : user.subscriptionRequest&&!changingRequest
            ...
            : plan==='TRIAL'
              ...
              : 'Puedes renovar el plan actual o solicitar un cambio para el próximo período.';
  ...
  </ScrollView><StaffManager open={staffOpen} currentUserId={user.id} requesterName={user.name} gymName={user.gym.name} onClose={()=>setStaffOpen(false)}/></>;
}

function StaffManager({ open, currentUserId, requesterName, gymName, onClose }: { open: boolean; currentUserId: string; requesterName: string; gymName: string; onClose: () => void }) {
  ...
  const load=useCallback(()=>{setLoading(true);api.staff().then(setAccounts).catch(showError).finally(()=>setLoading(false));},[]);
  useEffect(()=>{if(open){setFormOpen(false);setSelected(null);load();}},[open,load]);
  const begin=(account:StaffAccount|null)=>{if(account?.role==='ADMIN')return;setSelected(account);setName(account?.name??'');setEmail(account?.email??'');setPassword('');setIsActive(account?.isActive??true);setFormOpen(true);};
  const close=()=>{Keyboard.dismiss();if(formOpen){setFormOpen(false);setSelected(null);}else onClose();};
  const save=async()=>{setSaving(true);try{if(!await hasInternetConnection())throw new Error('Conéctate a internet para gestionar las cuentas del personal');const input={name:name.trim(),email:email.trim(),...(password?{password}:{}),...(selected?{isActive}:{})};if(selected)await api.updateStaff(selected.id,input);else await api.createStaff({...input,password});showSuccess(selected?'Recepcionista actualizado.':'Recepcionista creado.');setFormOpen(false);setSelected(null);load();}catch(error){showError(error);}finally{setSaving(false);}};
  ...
  const remove=()=>{if(!selected)return;Alert.alert('Eliminar cuenta',`¿Deseas eliminar la cuenta de ${selected.name}? Si tiene cobros registrados se desactivará para conservar el historial.`,[{text:'Cancelar',style:'cancel'},{text:'Eliminar',style:'destructive',onPress:()=>{void api.deleteStaff(selected.id).then(result=>{showSuccess(result.disposition==='DELETED'?'Cuenta eliminada.':'Cuenta desactivada para conservar el historial.');setFormOpen(false);setSelected(null);load();}).catch(showError);}}]);};
  const valid=!!name.trim()&&/^\S+@\S+\.\S+$/.test(email.trim())&&(selected?!password||password.length>=8:password.length>=8);
  return <Sheet open={open} title={formOpen?(selected?'Editar recepcionista':'Nuevo recepcionista'):'Personal y accesos'} onClose={close} footer={<PrimaryButton label={formOpen?(saving?'Guardando…':selected?'Guardar cambios':'Crear recepcionista'):'Añadir recepcionista'} icon={formOpen?'checkmark':'person-add-outline'} disabled={formOpen&&(!valid||saving)} onPress={()=>formOpen?void save():begin(null)}/> }>
    {formOpen?<>
