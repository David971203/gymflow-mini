...
function MembersScreen({ scope, revision }: ScreenProps) {
  ...
  const [loading, setLoading] = useState(true); const [addOpen, setAddOpen] = useState(false); const [assigning, setAssigning] = useState<Member | null>(null);
  ...
  return <>
    <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} />} contentContainerStyle={styles.scroll}>
      ...
      <SectionTitle title={`${members.length} miembros`} subtitle="Toca un miembro sin plan para asignarle uno" />
      <View style={styles.card}>{members.map(member => { const current = member.memberships[0]; return <Pressable key={member.id} onPress={() => setAssigning(member)} style={styles.memberRow}>
    ...
    <MemberForm scope={scope} open={addOpen} onClose={() => setAddOpen(false)} onSaved={() => { setAddOpen(false); load(); }} />
...
function MemberForm({ scope, open, onClose, onSaved }: FormProps) {
  const [ci, setCi] = useState(''); const [firstName, setFirstName] = useState(''); const [lastName, setLastName] = useState(''); const [phone, setPhone] = useState('');
  return <Sheet open={open} title="Nuevo miembro" onClose={onClose}><Field label="Carnet de identidad (11 dígitos)" value={ci} onChangeText={(value) => setCi(value.replace(/\D/g, '').slice(0, 11))} keyboardType="number-pad" maxLength={11} /><Field label="Nombre" value={firstName} onChangeText={setFirstName} /><Field label="Apellidos" value={lastName} onChangeText={setLastName} /><Field label="Teléfono" value={phone} onChangeText={setPhone} keyboardType="phone-pad" /><PrimaryButton label="Guardar miembro" onPress={() => offline.createMember(scope, { ci, firstName, lastName, phone }).then(onSaved).catch(showError)} disabled={ci.length !== 11 || !firstName || !lastName} /></Sheet>;
}
