...
function MembersScreen({ scope, revision, initialFilter }: ScreenProps & { initialFilter: MemberFilter }) {
  ...
  const confirmDeleteScheduled = (member: Member) => { const scheduled=scheduledMembership(member); if (!scheduled) return; Alert.alert('Eliminar renovación', `¿Deseas eliminar la renovación programada del plan ${scheduled.plan.name}? La membresía actual no cambiará.`, [{text:'Cancelar',style:'cancel'},{text:'Eliminar',style:'destructive',onPress:()=>{ try { ensureActiveSubscription(); void offline.deleteScheduledMembership(scope,member.id,scheduled.id).then(()=>{setSelected(null);load();}).catch(showError); } catch(error) { showError(error); } }}]); };
