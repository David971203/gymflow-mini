...
function MembersScreen({ scope, revision }: ScreenProps) {
  ...
  const [loading, setLoading] = useState(true); const [addOpen, setAddOpen] = useState(false); const [selected, setSelected] = useState<Member | null>(null); const [editing, setEditing] = useState<Member | null>(null); const [assigning, setAssigning] = useState<Member | null>(null);
  ...
  return <>
    <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} />} contentContainerStyle={styles.scroll}>
      ...
      <SectionTitle title={`${members.length} miembros`} subtitle="Toca un miembro para ver sus opciones" />
      <View style={styles.card}>{members.map(member => { const current = editableMembership(member) ?? member.memberships[0]; return <Pressable accessibilityRole="button" accessibilityLabel={`Opciones de ${member.firstName} ${member.lastName}`} onPress={() => setSelected(member)} key={member.id} style={({pressed}) => [styles.memberRow, pressed && styles.memberRowPressed]}>
      ...
      </Pressable>; })}{!members.length && <Empty text="Registra tu primer miembro" />}</View>
    </ScrollView>
...
function PlansScreen({ scope, revision }: ScreenProps) {
  ...
  const refresh = useCallback(() => { void syncNow(scope).then(load); }, [scope, load]);
  const confirmDelete = (plan: Plan) => Alert.alert('Eliminar plan', `¿Deseas eliminar el plan ${plan.name}? No podrá eliminarse si tiene membresías activas.`, [{ text:'Cancelar', style:'cancel' }, { text:'Eliminar', style:'destructive', onPress:() => { void offline.deletePlan(scope, plan.id).then(() => { setSelected(null); load(); }).catch(showError); } }]);
  useEffect(load, [load, revision]);
  return <><ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} />} contentContainerStyle={styles.scroll}>
    <PrimaryButton label="+ Crear plan" onPress={() => setOpen(true)} />
    <SectionTitle title="Planes del gimnasio" subtitle="Toca un plan para editarlo o eliminarlo" />
  ...
  </ScrollView><PlanActions plan={selected} onClose={() => setSelected(null)} onEdit={() => { if (selected) setEditing(selected); setSelected(null); }} onDelete={() => { if (selected) confirmDelete(selected); }}/><PlanForm scope={scope} open={open || !!editing} plan={editing} onClose={() => { setOpen(false); setEditing(null); }} onSaved={() => { setOpen(false); setEditing(null); load(); }} /></>;
}
...
function PlanForm({ scope, open, plan, onClose, onSaved }: FormProps & { plan: Plan | null }) {
  ...
  const save = async () => { setSaving(true); try { const input={ name:name.trim(), price:Number(price), durationDays:Number(days), description:description.trim(), isActive }; if (plan) await offline.updatePlan(scope, plan.id, input); else await offline.createPlan(scope, input); onSaved(); } catch(error) { showError(error); } finally { setSaving(false); } };
