...
async function exportPaymentsExcel(payments: Payment[], period: string) {
  ...
  file.write(bytes);
  Alert.alert('Excel descargado', `${fileName} se guardó correctamente en la carpeta seleccionada.`);
}
...
function AccountScreen({ user, themePreference, onThemeChange, onLogout }: { user: User; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void }) {
  ...
  const actions: Array<{label:string;message:string}> = !plan
    ...
    : expired
      ? [{ label:'Renovar por WhatsApp', message:`Hola, quiero renovar la membresía vencida del gimnasio ${user.gym.name}.` }]
      : plan==='TRIAL'
        ? [
            { label:'Adquirir plan mensual', message:`Hola, quiero adquirir el plan mensual para el gimnasio ${user.gym.name}. La prueba gratuita vence el ${expiry}.` },
            { label:'Adquirir plan anual', message:`Hola, quiero adquirir el plan anual para el gimnasio ${user.gym.name}. La prueba gratuita vence el ${expiry}.` },
          ]
        : plan==='MONTHLY'
          ? [
              { label:'Renovar plan mensual', message:`Hola, quiero renovar el plan mensual del gimnasio ${user.gym.name}, que vence el ${expiry}.` },
              { label:'Cambiar a plan anual', message:`Hola, quiero cambiar el gimnasio ${user.gym.name} del plan mensual al plan anual cuando termine el período actual el ${expiry}.` },
            ]
          : [
              { label:'Renovar plan anual', message:`Hola, quiero renovar el plan anual del gimnasio ${user.gym.name}, que vence el ${expiry}.` },
              { label:'Cambiar a plan mensual', message:`Hola, quiero cambiar el gimnasio ${user.gym.name} del plan anual al plan mensual cuando termine el período actual el ${expiry}.` },
            ];
  const status = !plan ? 'Sin plan' : expired ? 'Vencida' : 'Activa';
  const actionCopy = !plan ? 'Activa una membresía para habilitar nuevamente todas las operaciones.' : expired ? renewalMessage : plan==='TRIAL' ? 'Tu prueba está activa. Puedes adquirir un plan sin perder los días restantes.' : 'Puedes renovar el plan actual o solicitar un cambio para el próximo período.';
  return <ScrollView contentContainerStyle={styles.accountScroll}>
    <View style={styles.accountHero}><Text style={styles.accountHeroLabel}>GIMNASIO</Text><Text style={styles.accountHeroGym}>{user.gym.name}</Text></View>
    <View style={[styles.subscriptionCard,expired&&styles.subscriptionCardExpired]}>
      <View style={styles.subscriptionHead}><View style={[styles.subscriptionIcon,expired&&styles.subscriptionIconExpired]}><Ionicons name={expired?'alert-circle-outline':'shield-checkmark-outline'} size={23} color={expired?palette.danger:palette.action}/></View><View style={styles.rowMain}><Text style={styles.subscriptionEyebrow}>MEMBRESÍA DE GYMFLOW MINI</Text><Text style={[styles.subscriptionName,expired&&styles.subscriptionNameExpired]}>{gymSubscriptionLabel(plan)}</Text></View><View style={[styles.subscriptionBadge,expired&&styles.subscriptionBadgeExpired]}><Text style={[styles.subscriptionBadgeText,expired&&styles.subscriptionBadgeTextExpired]}>{status}</Text></View></View>
      <View style={styles.subscriptionDates}><View><Text style={styles.subscriptionDateLabel}>FECHA DE VENCIMIENTO</Text><Text style={styles.subscriptionDateValue}>{expiry}</Text></View>{!expired?<Text style={styles.subscriptionRemaining}>{remaining} día{remaining===1?'':'s'} restante{remaining===1?'':'s'}</Text>:null}</View>
      <Text style={[styles.subscriptionActionCopy,expired&&styles.subscriptionExpiredCopy]}>{actionCopy}</Text>
      <View style={styles.subscriptionActionGroup}>{actions.map((action,index)=><Pressable key={action.label} accessibilityRole="link" accessibilityLabel={`${action.label} por WhatsApp`} onPress={()=>openWhatsApp(action.message)} style={({pressed}) => [styles.whatsappButton,styles.whatsappButtonInGroup,index>0&&styles.whatsappButtonSecondary,pressed&&styles.tabPressed]}><Ionicons name="logo-whatsapp" size={19} color={index>0?palette.action:palette.white}/><Text style={[styles.whatsappButtonText,index>0&&styles.whatsappButtonTextSecondary]}>{action.label}</Text></Pressable>)}</View>
    </View>
...
function MembersScreen({ scope, revision, initialFilter }: ScreenProps & { initialFilter: MemberFilter }) {
  ...
  const confirmDelete = (member: Member) => Alert.alert('Eliminar miembro', `¿Deseas eliminar a ${member.firstName} ${member.lastName}? Se archivará como inactivo, se cancelarán su membresía y cobro activos, y todo quedará visible en el historial.`, [{ text:'Cancelar', style:'cancel' }, { text:'Eliminar', style:'destructive', onPress:() => { try { ensureActiveSubscription(); void offline.deleteMember(scope, member.id).then(() => { setSelected(null); load(); }).catch(showError); } catch(error) { showError(error); } } }]);
...
function PaymentsScreen({ scope, revision }: ScreenProps) {
  ...
  return <><ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} colors={[activeDarkTheme ? '#c9f47b' : '#1d6b4d']} tintColor={activeDarkTheme ? '#c9f47b' : '#1d6b4d'} progressBackgroundColor={activeDarkTheme ? '#212923' : '#fff'} />} contentContainerStyle={styles.scroll}>
    ...
    <View style={styles.paymentList}>{filteredPayments.map(payment => {
      ...
      return <Pressable disabled={disabled} onPress={() => withActiveSubscription(() => setSelected(payment))} key={payment.id} style={({pressed}) => [styles.paymentDetailRow,payment.status==='CANCELLED'&&styles.paymentDetailRowCancelled,pressed && styles.paymentDetailRowPressed]}>
...
const baseStyles = StyleSheet.create({
  ...
  periodStepper:{height:54,marginBottom:16,padding:5,flexDirection:'row',alignItems:'center',borderWidth:1,borderColor:palette.line,borderRadius:13,backgroundColor:'#fbfcfb'},periodStepButton:{width:44,height:42,alignItems:'center',justifyContent:'center',borderRadius:10,backgroundColor:'#e8f5ce'},periodStepValue:{flex:1,alignItems:'center',justifyContent:'center'},periodStepNumber:{color:palette.ink,fontSize:18,fontWeight:'900'},periodStepCaption:{marginTop:1,color:palette.secondary,fontSize:7,fontWeight:'900',letterSpacing:.9},purchasePreview:{marginBottom:16,padding:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:14,borderWidth:1,borderColor:'#cfe3d6',borderRadius:13,backgroundColor:'#f4faf6'},purchasePreviewLabel:{color:palette.secondary,fontSize:8,fontWeight:'900',letterSpacing:.8},purchasePreviewValue:{marginTop:5,color:palette.action,fontSize:16,fontWeight:'900'},purchasePreviewMeta:{marginTop:3,color:palette.secondary,fontSize:9,fontWeight:'700'},purchasePreviewDate:{marginTop:5,color:palette.ink,fontSize:11,fontWeight:'900',textAlign:'right'},
});
