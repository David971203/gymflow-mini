...
import { cancelMembershipNotifications, cancelSubscriptionNotifications, MEMBERSHIP_NOTIFICATION_SOURCE, SUBSCRIPTION_NOTIFICATION_SOURCE, supportsMembershipNotifications, syncMembershipNotifications, syncSubscriptionNotification } from './src/notifications';
import { discardSyncIssue, getSyncIssues, getSyncState, initializeOffline, offline, settleMutation, subscribeOffline, syncNow } from './src/offline';
...
import { effectiveMembershipStatus, isMembershipDayBefore, isMembershipDayOnOrBefore, membershipRemainingDays, shiftMembershipDayStart } from './src/membershipDates';
import type { Currency, Dashboard, GymSubscriptionPlan, Member, Membership, MemberSex, Payment, Plan, StaffAccount, SyncIssue, SyncState, Tab, User } from './src/types';

let activeCurrency: Currency = 'CUP';
let activeSubscriptionEndsAt = '';
let activeSubscriptionScope = '';
let activeSubscriptionTrialDays = 7;
const renewalMessage = 'Para continuar debes renovar la suscripción de tu gimnasio.';
...
const cubaLocations: Record<string,string[]> = {
...
const cubanProvinces=Object.keys(cubaLocations);
...
function subscriptionAccess(): { status: SubscriptionAccess; now: number } {
  ...
  if (!activeSubscriptionEndsAt || !Number.isFinite(endsAt)) return { status:'EXPIRED', now:clock.now };
  if (isMembershipDayBefore(activeSubscriptionEndsAt, clock.now)) return { status:'EXPIRED', now:clock.now };
  if (isMembershipDayOnOrBefore(activeSubscriptionEndsAt, clock.now)) return { status:'EXPIRING_TODAY', now:clock.now };
  return { status:'ACTIVE', now:clock.now };
}
function subscriptionIsActive() { const status = subscriptionAccess().status; return status === 'ACTIVE' || status === 'EXPIRING_TODAY'; }
...
function gymSubscriptionLabel(plan: GymSubscriptionPlan | null, trialDays = activeSubscriptionTrialDays) { return !plan?'Sin suscripción':plan==='TRIAL'?`Prueba gratuita · ${trialDays} días`:({MONTHLY:'Mensual',ANNUAL:'Anual'} as Record<Exclude<GymSubscriptionPlan,'TRIAL'>,string>)[plan]; }
  ...
  const rows: SheetData = [
    ...
    ...payments.map((payment, index) => {
      ...
      return [
        ...
        {value:`=MAX(0,E${row}-F${row})`,type:'Formula' as const,format:currencyFormat},
        status,
...
const ONBOARDING_STORAGE_KEY = 'gymflow_mini_onboarding_v1';
const SHOW_THEME_SELECTOR = false;
const SHOW_STAFF_ENTRY = false;
...
export default function App() {
  ...
  useEffect(() => {
    Promise.all([api.restore().catch(() => null), SecureStore.getItemAsync(THEME_STORAGE_KEY).catch(() => null), SecureStore.getItemAsync(ONBOARDING_STORAGE_KEY).catch(() => null)]).then(([restoredUser, storedTheme, storedOnboarding]) => {
      setUser(restoredUser);
      setOnboardingComplete(Boolean(restoredUser) || storedOnboarding === 'complete');
      if (restoredUser && storedOnboarding !== 'complete') void SecureStore.setItemAsync(ONBOARDING_STORAGE_KEY, 'complete').catch(() => undefined);
  ...
  const finishOnboarding = () => { setOnboardingComplete(true); void SecureStore.setItemAsync(ONBOARDING_STORAGE_KEY, 'complete').catch(() => undefined); };
      ...
      : !user
        ...
        : !user.gym.subscriptionPlan
          ? user.role === 'ADMIN'
            ...
            : <StaffSubscriptionBlocked user={user} onLogout={logout}/>
...
function OnboardingDashboardPreview() {
  return <View style={styles.onboardingDashboard}>
    <View style={styles.onboardingDashboardHead}><View><Text style={styles.onboardingDashboardGym}>HABANA FITNESS</Text><Text style={styles.onboardingDashboardTitle}>Inicio</Text></View><View style={styles.onboardingDashboardAvatar}><Text style={styles.onboardingDashboardAvatarText}>GF</Text></View></View>
    <View style={styles.onboardingSync}><View style={styles.onboardingSyncDot}/><Text style={styles.onboardingSyncText}>Datos al día</Text><Ionicons name="sync-outline" size={15} color={palette.action}/></View>
    <View style={styles.onboardingIncome}><Text style={styles.onboardingIncomeLabel}>INGRESOS DEL MES</Text><Text style={styles.onboardingIncomeValue}>248 500 CUP</Text><Text style={styles.onboardingIncomeCopy}>Dinero realmente cobrado</Text></View>
    <View style={styles.onboardingMetricRow}><View style={styles.onboardingMetric}><Text style={styles.onboardingMetricLabel}>MIEMBROS</Text><Text style={styles.onboardingMetricValue}>128</Text></View><View style={styles.onboardingMetric}><Text style={styles.onboardingMetricLabel}>ACTIVAS</Text><Text style={styles.onboardingMetricValue}>94</Text></View></View>
    <View style={styles.onboardingAlert}><View style={styles.onboardingAlertIcon}><Ionicons name="notifications-outline" size={18} color={palette.warning}/></View><View style={styles.onboardingAlertBody}><Text style={styles.onboardingAlertTitle}>Próximas a vencer</Text><Text style={styles.onboardingAlertCopy}>6 membresías requieren atención</Text></View><Ionicons name="chevron-forward" size={17} color={palette.secondary}/></View>
  </View>;
}

function OnboardingVisual({ type }: { type: OnboardingSlide['visual'] }) {
  if (type === 'dashboard') return <OnboardingDashboardPreview/>;
  if (type === 'features') return <View style={styles.onboardingFeatureGrid}>
    {[{icon:'people-outline' as const,label:'Miembros',copy:'Todo su historial'},{icon:'pricetags-outline' as const,label:'Planes',copy:'Vigencias claras'},{icon:'cash-outline' as const,label:'Caja',copy:'Cobros y deuda'},{icon:'notifications-outline' as const,label:'Avisos',copy:'Vence a tiempo'}].map(item=><View key={item.label} style={styles.onboardingFeatureCard}><View style={styles.onboardingFeatureIcon}><Ionicons name={item.icon} size={25} color={palette.action}/></View><Text style={styles.onboardingFeatureTitle}>{item.label}</Text><Text style={styles.onboardingFeatureCopy}>{item.copy}</Text></View>)}
  </View>;
  return <View style={styles.onboardingOfflineVisual}>
    <View style={styles.onboardingPhone}><View style={styles.onboardingPhoneSpeaker}/><Ionicons name="phone-portrait-outline" size={54} color={palette.action}/><Text style={styles.onboardingPhoneTitle}>Cambios guardados</Text><Text style={styles.onboardingPhoneCopy}>Listos para sincronizar</Text></View>
    <View style={styles.onboardingConnectionLine}><View style={styles.onboardingConnectionDot}/><View style={styles.onboardingConnectionDash}/><View style={styles.onboardingConnectionDot}/></View>
    <View style={styles.onboardingCloud}><Ionicons name="cloud-done-outline" size={48} color={palette.brand}/><Text style={styles.onboardingCloudTitle}>Todo al día</Text><Text style={styles.onboardingCloudCopy}>Al volver Internet</Text></View>
  </View>;
}
...
function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const last = page === onboardingSlides.length - 1;
  useEffect(() => {
    if (Platform.OS !== 'android' || page === 0) return;
    const subscription = BackHandler.addEventListener('hardwareBackPress', () => { setPage(current => Math.max(0, current - 1)); return true; });
    return () => subscription.remove();
  }, [page]);
  return <SafeAreaView style={styles.onboardingPage} edges={['top','right','bottom','left']}>
    <ExpoStatusBar style="dark" backgroundColor={palette.background}/>
    <View style={styles.onboardingTop}><Text style={styles.onboardingBrand}>GymFlow <Text style={styles.onboardingMini}>MINI</Text></Text><Pressable accessibilityRole="button" accessibilityLabel="Saltar introducción" onPress={onComplete} style={styles.onboardingSkip}><Text style={styles.onboardingSkipText}>Saltar</Text></Pressable></View>
    <View style={styles.onboardingBody}><View style={styles.onboardingVisualShell}><OnboardingVisual type={slide.visual}/></View><Text style={styles.onboardingEyebrow}>{slide.eyebrow}</Text><Text style={styles.onboardingTitle}>{slide.title}</Text><Text style={styles.onboardingCopy}>{slide.copy}</Text></View>
    <View style={styles.onboardingFooter}><View accessibilityLabel={`Pantalla ${page + 1} de ${onboardingSlides.length}`} style={styles.onboardingDots}>{onboardingSlides.map((item,index)=><View key={item.eyebrow} style={[styles.onboardingDot,index===page&&styles.onboardingDotActive]}/>)}</View><Pressable accessibilityRole="button" accessibilityLabel={last?'Ir al inicio de sesión':'Continuar'} onPress={()=>last?onComplete():setPage(current=>current+1)} style={({pressed})=>[styles.onboardingButton,pressed&&styles.tabPressed]}><Text style={styles.onboardingButtonText}>{last?'Comenzar':'Continuar'}</Text><Ionicons name={last?'arrow-forward':'chevron-forward'} size={19} color={palette.white}/></Pressable></View>
  </SafeAreaView>;
}
...
function Register({ onRegistered, onVerificationRequired, onBack }: { onRegistered: (user: User) => void; onVerificationRequired:(email:string)=>void; onBack: () => void }) {
  ...
  return <SafeAreaView style={styles.memberFormPage} edges={['top','right','bottom','left']}><ExpoStatusBar style={activeDarkTheme?'light':'dark'}/><KeyboardAvoidingView behavior={Platform.OS==='ios'?'padding':'height'} style={styles.loginKeyboardAvoiding}>
    ...
    <ScrollView ref={scrollRef} key={page} style={styles.memberFormScroll} contentContainerStyle={[styles.memberFormContent,keyboardOpen&&styles.memberFormContentKeyboard]} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS==='ios'?'interactive':'on-drag'} showsVerticalScrollIndicator={keyboardOpen}><KeyboardScrollContext.Provider value={reveal}>
      ...
      {page===1?<><Field label="Nombre del gimnasio" value={gymName} onChangeText={setGymName} autoCapitalize="words"/><ProvinceField value={province} onChange={value=>{setProvince(value);setMunicipality('');}}/><MunicipalityField province={province} value={municipality} onChange={setMunicipality}/></>:null}
...
function SubscriptionSetup({ user, onUserChange, onLogout }: { user: User; onUserChange: (user: User) => void; onLogout: () => void }) {
  ...
  const openWhatsApp=(current:User)=>{if(!renewalWhatsApp){showError(new Error('Configura el número de WhatsApp de GymFlow Mini.'));return;}const pending=current.subscriptionRequest;if(!pending)return;const message=pending.plan==='TRIAL'?`Hola, soy ${current.name}. Solicito verificar manualmente mi teléfono +53 ${current.phone??'no indicado'} para activar la prueba de ${current.gym.name}. Código ${pending.code}. Estoy escribiendo desde el mismo número registrado.`:`Hola, soy ${current.name}. Solicito el plan ${pending.plan==='MONTHLY'?'mensual':'anual'} para ${current.gym.name}. Código ${pending.code}. Teléfono ${current.phone??'no indicado'} y correo ${current.email}.`;void Linking.openURL(`https://wa.me/${renewalWhatsApp}?text=${encodeURIComponent(message)}`).catch(()=>showError(new Error('Comprueba que WhatsApp esté instalado.')));};
  const choose=async(plan:GymSubscriptionPlan)=>{setLoading(plan);try{const updated=await api.selectSubscription(plan,'ACTIVATE');onUserChange(updated);if(plan==='TRIAL')openWhatsApp(updated);}catch(error){showError(error);}finally{setLoading(null);}};
  const resendTrial=async()=>{setLoading('TRIAL');try{const updated=await api.selectSubscription('TRIAL','ACTIVATE');onUserChange(updated);openWhatsApp(updated);}catch(error){showError(error);}finally{setLoading(null);}};
...
function AdminApp({ user, dark, themePreference, onThemeChange, onLogout }: { user: User; dark: boolean; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void }) {
  ...
  return <SafeAreaView style={styles.app} edges={['top','right','bottom','left']}><StatusBar translucent backgroundColor="transparent" barStyle={dark ? "light-content" : "dark-content"} />
    ...
    <View style={styles.content}>
      ...
      {tab === 'MIEMBROS' && <MembersScreen scope={scope} revision={revision} initialFilter={memberEntryFilter} canDelete={currentUser.role === 'ADMIN'} />}
      ...
      {tab === 'PLANES' && <PlansScreen scope={scope} revision={revision} readOnly={currentUser.role !== 'ADMIN'} />}
      {tab === 'CAJA' && <PaymentsScreen scope={scope} revision={revision} />}
      {tab === 'ESTADISTICAS' && <StatisticsScreen scope={scope} revision={revision} />}
      {tab === 'PERSONAL' && currentUser.role === 'ADMIN' && <StaffManager currentUserId={currentUser.id} requesterName={currentUser.name} gymName={currentUser.gym.name} />}
      {tab === 'CUENTA' && <AccountScreen user={currentUser} onUserChange={setCurrentUser} themePreference={themePreference} onThemeChange={onThemeChange} onLogout={onLogout} passwordOpen={accountPasswordOpen} onPasswordOpenChange={setAccountPasswordOpen} onOpenStaff={() => setTab('PERSONAL')} />}
    </View>
...
function StatisticsScreen({ scope, revision }: ScreenProps) {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} colors={[activeDarkTheme ? '#c9f47b' : '#1d6b4d']} tintColor={activeDarkTheme ? '#c9f47b' : '#1d6b4d'} progressBackgroundColor={activeDarkTheme ? '#212923' : '#fff'} />} contentContainerStyle={styles.scroll}>
    ...
    {page === 'CLIENTS' ? <>
      ...
      <SectionTitle title="Miembros con cobros exigibles" subtitle={`${statistics.clients.debts.length} miembro${statistics.clients.debts.length===1?'':'s'} · ${money(statistics.clients.totalDebt)} por cobrar hoy · ${money(statistics.clients.futureDebt)} futuros`}/>
...
function AccountScreen({ user, onUserChange, themePreference, onThemeChange, onLogout, passwordOpen, onPasswordOpenChange, onOpenStaff }: { user: User; onUserChange: (user: User) => void; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void; passwordOpen: boolean; onPasswordOpenChange: (open: boolean) => void; onOpenStaff: () => void }) {
  ...
  const runSubscriptionAction=async(action:(typeof actions)[number])=>{setRequestingPlan(action.plan);try{const updated=await api.selectSubscription(action.plan,action.action);onUserChange(updated);setChangingRequest(false);}catch(error){showError(error);}finally{setRequestingPlan(null);}};
  const refreshRequest=async()=>{const requestCode=user.subscriptionRequest?.code;setRequestingPlan(user.subscriptionRequest?.plan??'MONTHLY');try{const updated=await api.refreshProfile();onUserChange(updated);setChangingRequest(false);const resolved=requestCode&&updated.latestSubscriptionRequest?.code===requestCode?updated.latestSubscriptionRequest:null;if(resolved?.status==='REJECTED')showRejected('La solicitud fue rechazada. Tu plan anterior no sufrió cambios.');else if(resolved?.status==='CANCELLED')showError(new Error('La solicitud fue cancelada o reemplazada.'));else if(updated.subscriptionRequest?.code===requestCode)showPending('La solicitud continúa pendiente.');else if(resolved?.status==='APPROVED'&&updated.gym.scheduledSubscriptionPlan)showSuccess(`La operación fue aprobada. El plan ${updated.gym.scheduledSubscriptionPlan==='MONTHLY'?'mensual':'anual'} comenzará el ${formatDate(updated.gym.scheduledSubscriptionStartsAt??null)}.`);else if(resolved?.status==='APPROVED')showSuccess('La operación fue aprobada y tu plan está actualizado.');else showError(new Error('La solicitud ya no está pendiente. Actualiza o crea una nueva solicitud.'));}catch(error){showError(error);}finally{setRequestingPlan(null);}};
  ...
  return <><ScrollView contentContainerStyle={styles.accountScroll}>
    ...
    <Text style={styles.accountVersion}>© 2026 GYMFLOW MINI · TODOS LOS DERECHOS RESERVADOS · V{appVersion}</Text>
  </ScrollView><Sheet open={profileOpen} title="Mis datos" onClose={()=>setProfileOpen(false)}><View style={styles.accountCard}><AccountRow label="Nombre" value={user.name}/><AccountRow label="Correo" value={user.email}/><AccountRow label="Gimnasio" value={user.gym.name}/>{user.gym.municipality&&user.gym.province?<AccountRow label="Ubicación" value={`${user.gym.municipality}, ${user.gym.province}`}/>:null}<AccountRow label="Moneda" value={user.gym.currency} last/></View></Sheet></>;
}

type StaffSection = 'RECEPTIONIST' | 'ADMIN' | 'TRAINER';
const staffSections: { id: StaffSection; label: string; icon: TabIconName }[] = [
...
function StaffManager({ currentUserId, requesterName, gymName }: { currentUserId: string; requesterName: string; gymName: string }) {
  ...
  const load=useCallback(()=>{setLoading(true);setLoadError(false);api.staff().then(setAccounts).catch(error=>{setLoadError(true);showError(error);}).finally(()=>setLoading(false));},[]);
  useEffect(()=>{load();},[load]);
  const begin=(account:StaffAccount|null)=>{if(account && account.role!=='RECEPTIONIST')return;withActiveSubscription(()=>{setSelected(account);setName(account?.name??'');setEmail(account?.email??'');setPassword('');setIsActive(account?.isActive??true);setFormOpen(true);});};
  const close=()=>{if(saving)return;Keyboard.dismiss();setFormOpen(false);setSelected(null);};
  const save=async()=>{setSaving(true);try{ensureActiveSubscription();if(!await hasInternetConnection())throw new Error('Conéctate a internet para gestionar las cuentas del personal');const input={name:name.trim(),email:email.trim(),...(password?{password}:{}),...(selected?{isActive}:{})};if(selected)await api.updateStaff(selected.id,input);else await api.createStaff({...input,password});showSuccess(selected?'Recepcionista actualizado.':'Recepcionista creado.');setFormOpen(false);setSelected(null);load();}catch(error){showError(error);}finally{setSaving(false);}};
  const requestAdministrator=()=>{if(!renewalWhatsApp){showError(new Error('Configura el número de soporte de GymFlow Mini.'));return;}const message=`Hola, soy ${requesterName}, administrador de ${gymName}. Necesito solicitar otra cuenta de administrador para mi gimnasio.`;void Linking.openURL(`https://wa.me/${renewalWhatsApp}?text=${encodeURIComponent(message)}`).catch(()=>showError(new Error('Comprueba que WhatsApp esté instalado.')));};
  const remove=()=>{if(!selected||saving)return;if(!subscriptionIsActive()){showError(new Error(subscriptionErrorMessage()));return;}Alert.alert('Eliminar cuenta',`¿Deseas eliminar la cuenta de ${selected.name}? Si tiene cobros registrados se desactivará para conservar el historial.`,[{text:'Cancelar',style:'cancel'},{text:'Eliminar',style:'destructive',onPress:()=>{if(!subscriptionIsActive()){showError(new Error(subscriptionErrorMessage()));return;}setSaving(true);void api.deleteStaff(selected.id).then(result=>{showSuccess(result.disposition==='DELETED'?'Cuenta eliminada.':'Cuenta desactivada para conservar el historial.');setFormOpen(false);setSelected(null);load();}).catch(showError).finally(()=>setSaving(false));}}]);};
  ...
  return <>
    <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
      ...
      {section==='TRAINER'?<View style={[styles.statisticsCard,{marginBottom:16}]}><Text style={styles.securityTitle}>Entrenadores · Próximamente</Text><Text style={styles.sheetCopy}>Aquí podrás gestionar el equipo de entrenamiento cuando esta función esté disponible.</Text><Text style={styles.registrationNote}>Las cuentas de entrenador todavía no están habilitadas.</Text></View>:<>
        <View style={[styles.statisticsCard,{marginBottom:16}]}><Text style={styles.securityTitle}>{section==='RECEPTIONIST'?'Acceso de recepción':'Acceso de administración'}</Text><Text style={styles.sheetCopy}>{section==='RECEPTIONIST'?'Cada recepcionista entra con su propio correo y contraseña. Puede registrar y editar miembros, gestionar membresías, registrar cobros y asistencia, y consultar planes.':'Los administradores gestionan la operación del gimnasio, los planes, las estadísticas, la suscripción y las cuentas de recepción.'}</Text>{section==='RECEPTIONIST'?<Text style={styles.registrationNote}>No puede eliminar miembros, modificar planes, ver estadísticas ni gestionar la suscripción o el personal.</Text>:<Text style={styles.registrationNote}>Las cuentas de administrador se gestionan mediante soporte.</Text>}</View>
      ...
      {loading?<LoadingSkeleton rows={3}/>:sectionAccounts.map(account=><Pressable key={account.id} accessibilityRole="button" disabled={account.role!=='RECEPTIONIST'} onPress={()=>begin(account)} style={({pressed})=>[styles.memberActionCard,pressed&&styles.memberActionCardPressed]}><View style={styles.memberActionIcon}><Ionicons name={account.role==='ADMIN'?'shield-checkmark-outline':'person-outline'} size={20} color={palette.action}/></View><View style={styles.rowMain}><Text style={styles.memberActionTitle}>{account.name}{account.id===currentUserId?' · Tú':''}</Text><Text style={styles.memberActionCopy}>{account.email}</Text><Text style={[styles.memberActionCopy,!account.isActive&&styles.memberActionDeleteText]}>{account.role==='ADMIN'?'Administrador · Gestionado por soporte':'Recepcionista'} · {account.isActive?'Activa':'Inactiva'}</Text></View>{account.role==='RECEPTIONIST'?<Ionicons name="chevron-forward" size={21} color={palette.secondary}/>:null}</Pressable>)}
        ...
        {section==='RECEPTIONIST'?<View style={{marginTop:20,marginBottom:8}}><PrimaryButton label="Añadir recepcionista" icon="person-add-outline" onPress={()=>begin(null)} disabled={!subscriptionIsActive()}/></View>:<>
      <Pressable accessibilityRole="link" accessibilityLabel="Solicitar otro administrador por WhatsApp" onPress={requestAdministrator} style={({pressed})=>[styles.supportButton,pressed&&styles.tabPressed]}><View style={styles.supportIcon}><Ionicons name="logo-whatsapp" size={23} color={palette.white}/></View><View style={styles.rowMain}><Text style={styles.supportTitle}>Solicitar otro administrador</Text><Text style={styles.supportCopy}>Contacta con soporte para verificar la solicitud</Text></View><Ionicons name="open-outline" size={19} color={palette.white}/></Pressable>
        </>}
      </>}
    </ScrollView>
    <Sheet liftAboveKeyboard open={formOpen} title={selected?'Editar recepcionista':'Nuevo recepcionista'} onClose={close} footer={<PrimaryButton label={saving?'Guardando…':selected?'Guardar cambios':'Crear recepcionista'} icon="checkmark" disabled={!valid||saving} onPress={()=>void save()}/>}>
      ...
      <Field label="Nombre" value={name} onChangeText={setName}/><Field label="Correo" value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address"/><Field label={selected?'Nueva contraseña (opcional)':'Contraseña'} value={password} onChangeText={setPassword} secureTextEntry autoCapitalize="none"/>
...
function MembersScreen({ scope, revision, initialFilter, canDelete }: ScreenProps & { initialFilter: MemberFilter; canDelete: boolean }) {
  ...
  const confirmDelete = (member: Member) => Alert.alert('Archivar miembro', `¿Deseas archivar a ${member.firstName} ${member.lastName}? Quedará inactivo y se cancelarán sus membresías vigentes. Los cobros, importes y abonos conservarán su estado y seguirán visibles en Caja.`, [{ text:'Cancelar', style:'cancel' }, { text:'Archivar', style:'destructive', onPress:() => { try { ensureActiveSubscription(); void offline.deleteMember(scope, member.id).then(async operationId => { const status=await reportMutation(scope,operationId,'Miembro archivado. Sus cobros y abonos se conservaron.','Archivo guardado en este dispositivo. Se confirmará al recuperar conexión.'); if(status!=='REJECTED')setSelected(null); load(); }).catch(showError); } catch(error) { showError(error); } } }]);
  const confirmDeleteScheduled = (member: Member) => { const scheduled=scheduledMembership(member); if (!scheduled) return; Alert.alert('Eliminar renovación', `¿Deseas eliminar la renovación programada del plan ${contractedPlan(scheduled).name}? La membresía actual no cambiará.`, [{text:'Cancelar',style:'cancel'},{text:'Eliminar',style:'destructive',onPress:()=>{ try { ensureActiveSubscription(); void offline.deleteScheduledMembership(scope,member.id,scheduled.id).then(async operationId=>{const status=await reportMutation(scope,operationId,'Renovación eliminada.','El cambio quedó guardado en este dispositivo.');if(status!=='REJECTED')setSelected(null);load();}).catch(showError); } catch(error) { showError(error); } }}]); };
  ...
  return <>
    <FlatList
      ...
      ListHeaderComponent={<>
        ...
        <View style={styles.memberListHeading}><View style={styles.rowMain}><Text style={styles.sectionHeading}>{query ? `${visibleMembers.length} resultado${visibleMembers.length === 1 ? '' : 's'}` : `${visibleMembers.length} miembro${visibleMembers.length === 1 ? '' : 's'}`}</Text><Text style={styles.sectionCopy}>{memberFilter === 'EXPIRED' ? 'Miembros cuya última membresía ya venció' : memberFilter === 'UPCOMING' ? 'Membresías que vencen en los próximos 10 días' : query ? `Búsqueda dentro del filtro seleccionado` : "Toca un miembro para ver sus opciones"}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`Filtrar miembros. Filtro actual: ${selectedFilter.label}`} accessibilityState={{expanded:filterOpen}} onPress={() => setFilterOpen(true)} style={({pressed})=>[styles.memberFilterTrigger,memberFilter!=='ALL'&&styles.memberFilterTriggerActive,pressed&&styles.tabPressed]}><Ionicons name="options-outline" size={18} color={memberFilter!=='ALL'||activeDarkTheme?activeDarkTheme?darkPalette.action:palette.action:palette.secondary}/><Text style={[styles.memberFilterTriggerText,memberFilter!=='ALL'&&styles.memberFilterTriggerTextActive]}>{selectedFilter.label}</Text><View style={[styles.memberFilterCount,memberFilter!=='ALL'&&styles.memberFilterCountActive]}><Text style={[styles.memberFilterCountText,memberFilter!=='ALL'&&styles.memberFilterCountTextActive]}>{selectedFilter.count}</Text></View><Ionicons name="chevron-down" size={15} color={activeDarkTheme?darkPalette.secondary:palette.secondary}/></Pressable></View>
      </>}
    ...
    />
    <Modal visible={filterOpen} transparent animationType="fade" statusBarTranslucent onRequestClose={() => setFilterOpen(false)}><View style={styles.memberFilterModalRoot}><Pressable accessibilityRole="button" accessibilityLabel="Cerrar filtros" style={styles.memberFilterBackdrop} onPress={() => setFilterOpen(false)}/><View accessibilityViewIsModal style={styles.memberFilterMenu}><View style={styles.memberFilterMenuHead}><Text style={styles.memberFilterMenuTitle}>Filtrar miembros</Text><Pressable accessibilityRole="button" accessibilityLabel="Cerrar filtros" onPress={() => setFilterOpen(false)} style={styles.memberFilterMenuClose}><Ionicons name="close" size={19} color={activeDarkTheme?darkPalette.secondary:palette.secondary}/></Pressable></View>{filterOptions.map(option => { const active=memberFilter===option.id; return <Pressable key={option.id} accessibilityRole="radio" accessibilityState={{checked:active}} onPress={() => { setMemberFilter(option.id); setFilterOpen(false); }} style={({pressed})=>[styles.memberFilterMenuOption,active&&styles.memberFilterMenuOptionActive,pressed&&styles.tabPressed]}><View style={styles.memberFilterMenuOptionIcon}>{active?<Ionicons name="checkmark" size={17} color={activeDarkTheme?darkPalette.action:palette.action}/>:null}</View><Text style={[styles.memberFilterMenuOptionText,active&&styles.memberFilterMenuOptionTextActive]}>{option.label}</Text><View style={[styles.memberFilterCount,active&&styles.memberFilterCountActive]}><Text style={[styles.memberFilterCountText,active&&styles.memberFilterCountTextActive]}>{option.count}</Text></View></Pressable>;})}</View></View></Modal>
    <MemberActions member={selected} canDelete={canDelete} onClose={() => setSelected(null)} onEdit={() => withActiveSubscription(() => { if (selected) setEditing(selected); setSelected(null); })} onPlan={() => withActiveSubscription(() => { if (selected) setAssigning(selected); setSelected(null); })} onRenew={() => withActiveSubscription(() => { if (selected) setRenewing(selected); setSelected(null); })} onEditScheduled={() => withActiveSubscription(() => { if (selected) setEditingScheduled(selected); setSelected(null); })} onDeleteScheduled={() => withActiveSubscription(() => { if (selected) confirmDeleteScheduled(selected); })} onDelete={() => withActiveSubscription(() => { if (selected) confirmDelete(selected); })} />
...
function MemberActions({ member, canDelete, onClose, onEdit, onPlan, onRenew, onEditScheduled, onDeleteScheduled, onDelete }: { member: Member | null; canDelete: boolean; onClose: () => void; onEdit: () => void; onPlan: () => void; onRenew: () => void; onEditScheduled: () => void; onDeleteScheduled: () => void; onDelete: () => void }) {
  ...
  return <Sheet open={!!member} title={fullName} onClose={onClose}>
    ...
    <View style={styles.memberActionGrid}>
      <Pressable accessibilityRole="button" onPress={onEdit} style={({pressed})=>[styles.memberActionCard,pressed&&styles.memberActionCardPressed]}><View style={styles.memberActionIcon}><Ionicons name="create-outline" size={20} color={palette.action}/></View><View style={styles.rowMain}><Text style={styles.memberActionTitle}>Editar datos</Text><Text style={styles.memberActionCopy}>Nombre, CI, teléfono y dirección</Text></View><Ionicons name="chevron-forward" size={21} color={palette.secondary}/></Pressable>
      <Pressable accessibilityRole="button" onPress={onPlan} style={({pressed})=>[styles.memberActionCard,styles.memberActionCardPlan,pressed&&styles.memberActionCardPressed]}><View style={[styles.memberActionIcon,styles.memberActionIconPlan]}><Ionicons name={active?'close-circle-outline':hasMembershipHistory?'refresh-outline':'card-outline'} size={20} color={active?palette.danger:palette.action}/></View><View style={styles.rowMain}><Text style={[styles.memberActionTitle,active&&styles.memberActionDeleteText]}>{active?'Cancelar membresía':hasMembershipHistory?'Renovar membresía':'Asignar membresía'}</Text><Text style={styles.memberActionCopy}>{active?'La membresía activa solo puede cancelarse':hasMembershipHistory?'Crear un nuevo período para este miembro':'Crear la primera membresía de este miembro'}</Text></View><Ionicons name="chevron-forward" size={21} color={palette.secondary}/></Pressable>
      ...
      {canDelete&&member?.status==='ACTIVE' ? <Pressable accessibilityRole="button" accessibilityLabel="Archivar miembro" onPress={onDelete} style={({pressed})=>[styles.memberActionCard,styles.memberActionCardDelete,pressed&&styles.memberActionCardPressed]}><View style={[styles.memberActionIcon,styles.memberActionIconDelete]}><Ionicons name="archive-outline" size={20} color={palette.danger}/></View><View style={styles.rowMain}><Text style={[styles.memberActionTitle,styles.memberActionDeleteText]}>Archivar miembro</Text><Text style={styles.memberActionCopy}>Lo marca inactivo y conserva intactos sus cobros y abonos</Text></View></Pressable> : null}
    </View>
  </Sheet>;
}

function PlansScreen({ scope, revision, readOnly }: ScreenProps & { readOnly: boolean }) {
  ...
  const deactivate = (plan: Plan) => { void onlineAction(() => Alert.alert('Desactivar plan', `¿Deseas desactivar el plan ${plan.name}? No podrá asignarse a nuevas membresías. Las membresías actuales y sus cobros no cambiarán.`, [{ text:'Cancelar', style:'cancel' }, { text:'Desactivar', style:'destructive', onPress:() => { void offline.updatePlan(scope, plan.id, { isActive:false }).then(async operationId => { const status=await reportMutation(scope,operationId,'Plan desactivado correctamente.'); if(status!=='REJECTED')setSelected(null); load(); }).catch(showError); } }])); };
  const reactivate = (plan: Plan) => { void onlineAction(() => Alert.alert('Reactivar plan', `¿Deseas volver a activar el plan ${plan.name}? Estará disponible para nuevas membresías.`, [{ text:'Cancelar', style:'cancel' }, { text:'Reactivar', onPress:() => { void offline.updatePlan(scope, plan.id, { isActive:true }).then(async operationId => { const status=await reportMutation(scope,operationId,'Plan reactivado correctamente.'); if(status!=='REJECTED')setSelected(null); load(); }).catch(showError); } }])); };
  ...
  </ScrollView>{!readOnly ? <PlanActions plan={selected} onClose={() => setSelected(null)} onEdit={() => { void onlineAction(() => { if (selected) setEditing(selected); setSelected(null); }); }} onDeactivate={() => { if (selected) deactivate(selected); }} onReactivate={() => { if (selected) reactivate(selected); }} onDelete={() => { if (selected) confirmDelete(selected); }}/> : null}</>;
}
...
function PaymentsScreen({ scope, revision }: ScreenProps) {
  ...
  const outstandingPayments = payments.filter(payment => Number(payment.amount) > Number(payment.paidAmount));
  const dueDebt = outstandingPayments.filter(payment => paymentIsDue(payment)).reduce((sum,payment)=>sum+Math.max(0,Number(payment.amount)-Number(payment.paidAmount)),0);
  const overdueDebt = outstandingPayments.filter(payment => !!payment.dueDate && isMembershipDayBefore(payment.dueDate)).reduce((sum,payment)=>sum+Math.max(0,Number(payment.amount)-Number(payment.paidAmount)),0);
  const futureDebt = outstandingPayments.filter(payment => !!payment.dueDate && !paymentIsDue(payment)).reduce((sum,payment)=>sum+Math.max(0,Number(payment.amount)-Number(payment.paidAmount)),0);
  ...
  const filteredDebt = filteredPayments.reduce((sum, payment) => sum + Math.max(0, Number(payment.amount) - Number(payment.paidAmount)), 0);
  ...
  return <><FlatList
    ...
    renderItem={({ item:payment }) => {
      ...
      return <Pressable disabled={disabled} onPress={() => withActiveSubscription(() => setSelected(payment))} style={({pressed}) => [styles.paymentDetailRow,pressed && styles.paymentDetailRowPressed]}>
...
function MemberForm({ scope, open, member, plans, onClose, onSaved }: FormProps & { member: Member | null; plans: Plan[] }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (member) operationId=await offline.updateMember(scope, member.id, input);
      else {
        if (!planId) throw new Error('Selecciona el plan inicial');
        const created=await offline.createMemberWithMembership(scope,input,{ planId, periodCount, ...(Number(initialPayment)>0?{initialPayment:Number(initialPayment),paymentMethod:'CASH'}:{}) });
  ...
  const pageTitles = ['Datos personales', 'Identificación', 'Información adicional', 'Contacto', 'Plan inicial'];
  ...
  return <View style={styles.memberFormPage}>
    ...
    <ScrollView ref={scrollRef} key={page} style={styles.memberFormScroll} contentContainerStyle={[styles.memberFormContent, keyboardOpen && styles.memberFormContentKeyboard]} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} showsVerticalScrollIndicator={keyboardOpen}>
      ...
      {page === 3 ? <><Field label="Teléfono" value={phone} onChangeText={setPhone} keyboardType="phone-pad" /><Field label="Dirección" value={address} onChangeText={setAddress} />{member ? <Text style={styles.memberFormCopy}>El estado activo o inactivo se calcula automáticamente a partir de sus membresías.</Text> : null}</> : null}
...
function PlanActions({ plan, onClose, onEdit, onDeactivate, onReactivate, onDelete }: { plan: Plan | null; onClose: () => void; onEdit: () => void; onDeactivate: () => void; onReactivate: () => void; onDelete: () => void }) {
  return <Sheet open={!!plan} title="Opciones del plan" onClose={onClose}>
    ...
    <View style={styles.memberActionGrid}>
      ...
      {plan?.isActive ? <Pressable accessibilityRole="button" accessibilityLabel="Desactivar plan" onPress={onDeactivate} style={({pressed}) => [styles.memberActionCard, styles.memberActionCardDelete, pressed && styles.memberActionCardPressed]}><View style={[styles.memberActionIcon, styles.memberActionIconDelete]}><Ionicons name="pause-circle-outline" size={20} color={palette.danger}/></View><View style={styles.rowMain}><Text style={[styles.memberActionTitle, styles.memberActionDeleteText]}>Desactivar plan</Text><Text style={styles.memberActionCopy}>Impide nuevas asignaciones sin afectar membresías vigentes</Text></View></Pressable> : null}
...
function PlanForm({ scope, open, plan, onClose, onSaved }: FormProps & { plan: Plan | null }) {
  ...
  const save = async () => { setSaving(true); try { ensureActiveSubscription(); if (!await hasInternetConnection()) { showOnlineRequired(); return; } const input={ name:name.trim(), price:Number(price), durationDays:Number(days), description:description.trim(), isActive }; if (plan) { const operationId=await offline.updatePlan(scope, plan.id, input); const status=await reportMutation(scope,operationId,'Plan actualizado correctamente.'); if(status==='REJECTED')return; } else { await api.createPlan(input); await syncNow(scope); showSuccess('Plan creado correctamente.'); } Keyboard.dismiss(); onSaved(); } catch(error) { showError(error); } finally { setSaving(false); } };
...
function AssignPlanForm({ scope, member, plans, onClose, onSaved }: { scope: string; member: Member | null; plans: Plan[]; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => { if (!member || !planId) return; setSaving(true); try { ensureActiveSubscription(); const operationId=membership ? await offline.updateMembership(scope, member.id, membership.id, { planId, status:'CANCELLED' }) : (await offline.assignPlan(scope, { memberId: member.id, planId, periodCount, ...(Number(amount) > 0 ? { initialPayment: Number(amount), paymentMethod: 'CASH' } : {}) })).operationId; const status=await reportMutation(scope,operationId,membership?'Membresía cancelada. El cobro y sus importes se conservaron.':isRenewal?'Membresía renovada correctamente.':'Membresía asignada correctamente.'); if(status!=='REJECTED')onSaved(); } catch (error) { showError(error); } finally { setSaving(false); } };
  ...
  return <Sheet open={!!member} title={`${title} de ${member?.firstName ?? ''}`} onClose={onClose} footer={<PrimaryButton label={saving ? (membership?'Cancelando…':'Guardando…') : title} disabled={saving || !planId} onPress={submit} />}>
    ...
    {membership?<Text style={styles.sheetCopy}>Esta es la única acción disponible mientras la membresía está activa. Podrás renovarla después de cancelarla.</Text>:<><Text style={styles.fieldLabel}>{isRenewal?'Selecciona el plan para renovar':'Selecciona el plan'}</Text><View style={styles.choices}>{availablePlans.map(plan => <Pressable key={plan.id} onPress={() => setPlanId(plan.id)} style={[styles.choice, planId === plan.id && styles.choiceActive]}><Text style={styles.choiceTitle}>{plan.name}{isRenewal&&plan.id===previousMembership?.plan.id?' · Anterior':''}</Text><Text style={styles.choicePrice}>{money(plan.price)}</Text></Pressable>)}</View><PeriodSelector value={periodCount} onChange={setPeriodCount}/>{selectedPlan?<MembershipPurchasePreview plan={selectedPlan} periodCount={periodCount} endDate={previewEndDate}/>:null}<Field label="Abono inicial (opcional)" value={amount} onChangeText={setAmount} keyboardType="numeric" /></>}
  </Sheet>;
}
...
function RenewMembershipForm({ scope, member, plans, onClose, onSaved }: { scope: string; member: Member | null; plans: Plan[]; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => { if (!member || !current || !planId) return; setSaving(true); try { ensureActiveSubscription(); const {operationId}=await offline.renewMembership(scope,member.id,current.id,{ planId, periodCount, ...(Number(amount)>0?{initialPayment:Number(amount),paymentMethod:'CASH'}:{}) }); const status=await reportMutation(scope,operationId,'Renovación programada correctamente.'); if(status!=='REJECTED')onSaved(); } catch(error) { showError(error); } finally { setSaving(false); } };
...
function ScheduledMembershipForm({ scope, member, plans, onClose, onSaved }: { scope: string; member: Member | null; plans: Plan[]; onClose: () => void; onSaved: () => void }) {
  ...
  const save=async()=>{ if(!member||!scheduled||!planId)return; setSaving(true); try { ensureActiveSubscription(); const operationId=await offline.updateMembership(scope,member.id,scheduled.id,{planId,status:'SCHEDULED'}); const status=await reportMutation(scope,operationId,'Renovación actualizada correctamente.'); if(status!=='REJECTED')onSaved(); } catch(error) { showError(error); } finally { setSaving(false); } };
...
function PaymentForm({ scope, payment, onClose, onSaved }: { scope: string; payment: Payment | null; onClose: () => void; onSaved: () => void }) {
  const balance = payment ? Number(payment.amount) - Number(payment.paidAmount) : 0; const [amount, setAmount] = useState(''); const [saving,setSaving]=useState(false);
  useEffect(() => setAmount(payment ? String(balance) : ''), [payment, balance]);
  const save=async()=>{if(!payment)return;setSaving(true);try{ensureActiveSubscription();const operationId=await offline.applyPayment(scope,payment.id,{amount:Number(amount),method:'CASH'});const status=await reportMutation(scope,operationId,'Cobro registrado correctamente.','Cobro guardado en este dispositivo. Se confirmará al recuperar conexión.');if(status!=='REJECTED')onSaved();}catch(error){showError(error);}finally{setSaving(false);}};
  return <Sheet open={!!payment} title="Registrar abono" onClose={onClose} footer={<PrimaryButton label={saving?'Confirmando…':'Confirmar cobro'} disabled={saving||!amount || Number(amount) <= 0 || Number(amount) > balance} onPress={()=>void save()} />}><Text style={styles.sheetCopy}>Saldo de {payment?.member.firstName}: <Text style={styles.bold}>{money(balance)}</Text></Text><Field label="Importe recibido en efectivo" value={amount} onChangeText={setAmount} keyboardType="numeric" /></Sheet>;
}
...
function Sheet({ open, title, onClose, children, footer, liftAboveKeyboard = false }: { open: boolean; title: string; onClose: () => void; children: React.ReactNode; footer?: React.ReactNode; liftAboveKeyboard?: boolean }) {
  ...
  const updateKeyboardInset = useCallback(() => {
    ...
    // Measure the modal itself: Android may already have resized its window.
  ...
  return <Modal visible={mounted} transparent animationType="none" hardwareAccelerated statusBarTranslucent onRequestClose={onClose}>
    <View ref={modalRootRef} collapsable={false} style={styles.modalRoot} onLayout={updateKeyboardInset}>
      ...
      <KeyboardAvoidingView enabled={!liftAboveKeyboard} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 8 : 0} style={[keyboardFixStyles.keyboardAvoiding, liftAboveKeyboard && {paddingBottom:keyboardInset}]}>
      <Animated.View style={[styles.sheet, { paddingBottom:liftAboveKeyboard && keyboardOpen ? 12 : Math.max(20,insets.bottom + 12) }, !!displayedFooter.current && keyboardOpen && styles.formSheet, liftAboveKeyboard && keyboardOpen && {height:'96%',maxHeight:'96%'}, { opacity: progress, transform: [{ translateY: progress.interpolate({ inputRange:[0,1], outputRange:[52,0] }) }, { scale: progress.interpolate({ inputRange:[0,1], outputRange:[.985,1] }) }] }]}>
...
function ProvinceField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <View style={styles.field}><Text maxFontSizeMultiplier={1.35} style={styles.fieldLabel}>Provincia</Text><Pressable accessibilityRole="button" accessibilityLabel="Seleccionar provincia" accessibilityState={{expanded:open}} onPress={()=>{Keyboard.dismiss();setOpen(true);}} style={({pressed})=>[styles.input,styles.provinceTrigger,pressed&&styles.tabPressed]}><Text style={[styles.provinceValue,!value&&styles.provincePlaceholder]}>{value||'Seleccionar provincia'}</Text><Ionicons name="chevron-down" size={18} color={activeDarkTheme?darkPalette.secondary:palette.secondary}/></Pressable><Modal visible={open} transparent animationType="fade" statusBarTranslucent onRequestClose={()=>setOpen(false)}><View style={styles.provinceModalRoot}><Pressable accessibilityRole="button" accessibilityLabel="Cerrar selector" style={StyleSheet.absoluteFill} onPress={()=>setOpen(false)}/><SafeAreaView edges={['bottom']} style={styles.provinceSheet}><View style={styles.provinceHead}><Text style={styles.provinceTitle}>Selecciona tu provincia</Text><Pressable accessibilityRole="button" accessibilityLabel="Cerrar" onPress={()=>setOpen(false)} style={styles.closeButton}><Ionicons name="close" size={22} color={activeDarkTheme?darkPalette.text:palette.ink}/></Pressable></View><ScrollView showsVerticalScrollIndicator={false}>{cubanProvinces.map(province=><Pressable key={province} onPress={()=>select(province)} style={[styles.provinceOption,value===province&&styles.provinceOptionSelected]}><Text style={[styles.provinceOptionText,value===province&&styles.provinceOptionTextSelected]}>{province}</Text>{value===province&&<Ionicons name="checkmark" size={19} color={palette.action}/>}</Pressable>)}</ScrollView></SafeAreaView></View></Modal></View>;
}
function MunicipalityField({ province, value, onChange }: { province: string; value: string; onChange: (value: string) => void }) {
  const [open,setOpen]=useState(false);
  const municipalities=cubaLocations[province]??[];
  const select=(municipality:string)=>{onChange(municipality);setOpen(false);};
  return <View style={styles.field}><Text maxFontSizeMultiplier={1.35} style={styles.fieldLabel}>Municipio</Text><Pressable accessibilityRole="button" accessibilityLabel="Seleccionar municipio" accessibilityState={{expanded:open,disabled:!province}} disabled={!province} onPress={()=>{Keyboard.dismiss();setOpen(true);}} style={({pressed})=>[styles.input,styles.provinceTrigger,!province&&styles.disabled,pressed&&styles.tabPressed]}><Text style={[styles.provinceValue,!value&&styles.provincePlaceholder]}>{value||(province?'Seleccionar municipio':'Selecciona primero la provincia')}</Text><Ionicons name="chevron-down" size={18} color={activeDarkTheme?darkPalette.secondary:palette.secondary}/></Pressable><Modal visible={open} transparent animationType="fade" statusBarTranslucent onRequestClose={()=>setOpen(false)}><View style={styles.provinceModalRoot}><Pressable accessibilityRole="button" accessibilityLabel="Cerrar selector" style={StyleSheet.absoluteFill} onPress={()=>setOpen(false)}/><SafeAreaView edges={['bottom']} style={styles.provinceSheet}><View style={styles.provinceHead}><Text style={styles.provinceTitle}>Selecciona tu municipio</Text><Pressable accessibilityRole="button" accessibilityLabel="Cerrar" onPress={()=>setOpen(false)} style={styles.closeButton}><Ionicons name="close" size={22} color={activeDarkTheme?darkPalette.text:palette.ink}/></Pressable></View><ScrollView showsVerticalScrollIndicator={false}>{municipalities.map(municipality=><Pressable key={municipality} onPress={()=>select(municipality)} style={[styles.provinceOption,value===municipality&&styles.provinceOptionSelected]}><Text style={[styles.provinceOptionText,value===municipality&&styles.provinceOptionTextSelected]}>{municipality}</Text>{value===municipality&&<Ionicons name="checkmark" size={19} color={palette.action}/>}</Pressable>)}</ScrollView></SafeAreaView></View></Modal></View>;
}
...
function scheduledMembership(member: Member) { return member.memberships.find(membership => effectiveMembershipStatus(membership) === 'SCHEDULED'); }
function upcomingMembership(member: Member) { if (member.status !== 'ACTIVE') return undefined; const membership=activeMembership(member); if (!membership) return undefined; const days=membershipRemainingDays(membership.endDate); return days > 0 && days <= 10 ? membership : undefined; }
function editableMembership(member: Member) { return activeMembership(member); }
function hasExpiredMembership(member: Member) { const latest=member.memberships[0]; return !!latest && effectiveMembershipStatus(latest) === 'EXPIRED' && !activeMembership(member) && !scheduledMembership(member); }
...
function formatDate(value: string | null) { return value ? new Date(value).toLocaleDateString('es-CU', { timeZone:'America/Havana', day:'2-digit', month:'short', year:'numeric' }) : 'Sin fecha'; }
...
function remainingDaysLabel(value: string) { const days=membershipRemainingDays(value); return `${days} día${days === 1 ? '' : 's'} restante${days === 1 ? '' : 's'}`; }
...
const baseStyles = StyleSheet.create(withReadableType({
  ...
  onboardingPage:{flex:1,paddingHorizontal:22,paddingTop:12,paddingBottom:16,backgroundColor:palette.background},onboardingTop:{height:52,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},onboardingBrand:{color:palette.brand,fontSize:21,fontWeight:'900'},onboardingMini:{color:palette.action,fontSize:10,letterSpacing:2},onboardingSkip:{minWidth:64,minHeight:44,alignItems:'flex-end',justifyContent:'center'},onboardingSkipText:{color:palette.action,fontSize:13,fontWeight:'900'},onboardingBody:{flex:1,justifyContent:'center'},onboardingVisualShell:{height:'50%',minHeight:230,maxHeight:310,marginBottom:22,alignItems:'center',justifyContent:'center',overflow:'hidden',borderWidth:1,borderColor:'#dce7df',borderRadius:30,backgroundColor:'#edf5e8'},onboardingEyebrow:{color:palette.action,fontSize:10,fontWeight:'900',letterSpacing:1.6},onboardingTitle:{maxWidth:360,marginTop:8,color:palette.ink,fontSize:26,lineHeight:31,fontWeight:'900',letterSpacing:-.8},onboardingCopy:{maxWidth:370,marginTop:9,color:palette.secondary,fontSize:14,lineHeight:20,fontWeight:'600'},onboardingFooter:{paddingTop:10},onboardingDots:{height:24,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:7},onboardingDot:{width:7,height:7,borderRadius:4,backgroundColor:'#c9d2cc'},onboardingDotActive:{width:24,backgroundColor:palette.action},onboardingButton:{minHeight:54,marginTop:8,paddingHorizontal:20,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,borderRadius:16,backgroundColor:palette.action},onboardingButtonText:{color:palette.white,fontSize:15,fontWeight:'900'},
  onboardingDashboard:{width:'86%',maxWidth:330,padding:16,borderWidth:1,borderColor:'#dce4df',borderRadius:22,backgroundColor:palette.white,shadowColor:palette.brand,shadowOffset:{width:0,height:12},shadowOpacity:.14,shadowRadius:18,elevation:7,transform:[{rotate:'-1.5deg'},{scale:.82}]},onboardingDashboardHead:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},onboardingDashboardGym:{color:palette.secondary,fontSize:8,fontWeight:'900',letterSpacing:1},onboardingDashboardTitle:{marginTop:3,color:palette.ink,fontSize:19,fontWeight:'900'},onboardingDashboardAvatar:{width:38,height:38,alignItems:'center',justifyContent:'center',borderRadius:13,backgroundColor:palette.brand},onboardingDashboardAvatarText:{color:palette.accent,fontSize:11,fontWeight:'900'},onboardingSync:{height:32,marginTop:13,paddingHorizontal:10,flexDirection:'row',alignItems:'center',borderRadius:10,backgroundColor:'#f3f7f3'},onboardingSyncDot:{width:7,height:7,borderRadius:4,backgroundColor:palette.action},onboardingSyncText:{flex:1,marginLeft:6,color:palette.secondary,fontSize:9,fontWeight:'800'},onboardingIncome:{marginTop:10,padding:14,borderRadius:15,backgroundColor:palette.brand},onboardingIncomeLabel:{color:'#bcd2c7',fontSize:8,fontWeight:'900',letterSpacing:1},onboardingIncomeValue:{marginTop:5,color:palette.white,fontSize:21,fontWeight:'900'},onboardingIncomeCopy:{marginTop:3,color:palette.accent,fontSize:8,fontWeight:'700'},onboardingMetricRow:{marginTop:9,flexDirection:'row',gap:8},onboardingMetric:{flex:1,padding:11,borderWidth:1,borderColor:palette.line,borderRadius:13,backgroundColor:'#fbfcfb'},onboardingMetricLabel:{color:palette.secondary,fontSize:8,fontWeight:'900'},onboardingMetricValue:{marginTop:4,color:palette.ink,fontSize:17,fontWeight:'900'},onboardingAlert:{marginTop:9,padding:10,flexDirection:'row',alignItems:'center',borderWidth:1,borderColor:'#f1d2b2',borderRadius:13,backgroundColor:'#fff8ef'},onboardingAlertIcon:{width:32,height:32,alignItems:'center',justifyContent:'center',borderRadius:10,backgroundColor:'#fff0dc'},onboardingAlertBody:{flex:1,marginLeft:9},onboardingAlertTitle:{color:palette.ink,fontSize:9,fontWeight:'900'},onboardingAlertCopy:{marginTop:2,color:palette.secondary,fontSize:8,fontWeight:'600'},
  onboardingFeatureGrid:{width:'84%',maxWidth:320,flexDirection:'row',flexWrap:'wrap',gap:10},onboardingFeatureCard:{width:'48%',minHeight:116,padding:14,borderWidth:1,borderColor:'#dce4df',borderRadius:19,backgroundColor:palette.white,shadowColor:palette.brand,shadowOffset:{width:0,height:5},shadowOpacity:.07,shadowRadius:10,elevation:3},onboardingFeatureIcon:{width:43,height:43,alignItems:'center',justifyContent:'center',borderRadius:14,backgroundColor:'#e8f5ce'},onboardingFeatureTitle:{marginTop:10,color:palette.ink,fontSize:13,fontWeight:'900'},onboardingFeatureCopy:{marginTop:3,color:palette.secondary,fontSize:9,fontWeight:'600'},
  onboardingOfflineVisual:{width:'90%',maxWidth:340,flexDirection:'row',alignItems:'center',justifyContent:'center'},onboardingPhone:{width:132,height:205,paddingHorizontal:12,alignItems:'center',justifyContent:'center',borderWidth:5,borderColor:palette.brand,borderRadius:26,backgroundColor:palette.white,shadowColor:palette.brand,shadowOffset:{width:0,height:8},shadowOpacity:.13,shadowRadius:14,elevation:5},onboardingPhoneSpeaker:{position:'absolute',top:8,width:35,height:4,borderRadius:2,backgroundColor:'#c9d2cc'},onboardingPhoneTitle:{marginTop:10,color:palette.ink,fontSize:11,fontWeight:'900',textAlign:'center'},onboardingPhoneCopy:{marginTop:3,color:palette.secondary,fontSize:8,fontWeight:'700',textAlign:'center'},onboardingConnectionLine:{width:43,flexDirection:'row',alignItems:'center'},onboardingConnectionDot:{width:6,height:6,borderRadius:3,backgroundColor:palette.action},onboardingConnectionDash:{flex:1,height:2,backgroundColor:'#9fbea9'},onboardingCloud:{width:126,minHeight:132,padding:14,alignItems:'center',justifyContent:'center',borderWidth:1,borderColor:'#d4e2d8',borderRadius:24,backgroundColor:palette.accent},onboardingCloudTitle:{marginTop:8,color:palette.brand,fontSize:12,fontWeight:'900',textAlign:'center'},onboardingCloudCopy:{marginTop:3,color:'#3e604f',fontSize:8,fontWeight:'700',textAlign:'center'},
  ...
  memberFilters:{paddingTop:12,flexDirection:'row',flexWrap:'wrap',gap:8},memberFilter:{minHeight:44,paddingHorizontal:12,flexDirection:'row',alignItems:'center',gap:7,borderWidth:1,borderColor:palette.line,borderRadius:22,backgroundColor:palette.white},memberFilterActive:{borderColor:palette.action,backgroundColor:'#eef7e1'},memberFilterText:{color:palette.secondary,fontSize:12,fontWeight:'800'},memberFilterTextActive:{color:palette.action},memberListHeading:{marginTop:18,marginBottom:10,flexDirection:'row',alignItems:'flex-start',gap:12},memberFilterTrigger:{minHeight:38,paddingLeft:11,paddingRight:9,flexDirection:'row',alignItems:'center',gap:7,borderWidth:1,borderColor:palette.line,borderRadius:12,backgroundColor:palette.white},memberFilterTriggerActive:{borderColor:'#b9d8c5',backgroundColor:'#f4faf6'},memberFilterTriggerText:{color:palette.secondary,fontSize:11,fontWeight:'800'},memberFilterTriggerTextActive:{color:palette.action},memberFilterModalRoot:{flex:1,paddingTop:(StatusBar.currentHeight??24)+132,paddingHorizontal:16,alignItems:'flex-end'},memberFilterBackdrop:{...StyleSheet.absoluteFillObject,backgroundColor:'#13251d26'},memberFilterMenu:{width:260,padding:8,borderWidth:1,borderColor:palette.line,borderRadius:17,backgroundColor:palette.white,shadowColor:'#000',shadowOffset:{width:0,height:8},shadowOpacity:.2,shadowRadius:18,elevation:20},memberFilterMenuHead:{minHeight:42,paddingLeft:10,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},memberFilterMenuTitle:{color:palette.ink,fontSize:13,fontWeight:'900'},memberFilterMenuClose:{width:38,height:38,alignItems:'center',justifyContent:'center',borderRadius:10},memberFilterMenuOption:{minHeight:48,paddingHorizontal:8,flexDirection:'row',alignItems:'center',gap:8,borderRadius:11},memberFilterMenuOptionActive:{backgroundColor:'#eef7e1'},memberFilterMenuOptionIcon:{width:22,alignItems:'center'},memberFilterMenuOptionText:{flex:1,color:palette.ink,fontSize:12,fontWeight:'700'},memberFilterMenuOptionTextActive:{color:palette.action,fontWeight:'900'},memberFilterCount:{minWidth:24,height:24,paddingHorizontal:6,alignItems:'center',justifyContent:'center',borderRadius:12,backgroundColor:'#eef2ef'},memberFilterCountActive:{backgroundColor:palette.action},memberFilterCountText:{color:palette.secondary,fontSize:10,fontWeight:'900'},memberFilterCountTextActive:{color:palette.white},memberPlanDotUpcoming:{backgroundColor:palette.warning},memberPlanTextUpcoming:{color:palette.warning,fontWeight:'800'},memberPlanDotExpired:{backgroundColor:palette.warning},memberPlanTextExpired:{color:palette.warning},
...
const darkStyles = StyleSheet.create({
  ...
  memberFilterTrigger:{borderColor:darkPalette.border,backgroundColor:darkPalette.surface},
  memberFilterTriggerActive:{borderColor:darkPalette.action,backgroundColor:darkPalette.actionSoft},
  memberFilterTriggerText:{color:darkPalette.secondary},
  memberFilterTriggerTextActive:{color:darkPalette.action},
  memberFilterBackdrop:{backgroundColor:'#05090799'},
  memberFilterMenu:{borderColor:darkPalette.border,backgroundColor:darkPalette.surface,shadowColor:'#000'},
  memberFilterMenuTitle:{color:darkPalette.text},
  memberFilterMenuOptionActive:{backgroundColor:darkPalette.actionSoft},
  memberFilterMenuOptionText:{color:darkPalette.text},
  memberFilterMenuOptionTextActive:{color:darkPalette.action},
