...
function AccountScreen({ user, onUserChange, themePreference, onThemeChange, onLogout, passwordOpen, onPasswordOpenChange, onOpenStaff }: { user: User; onUserChange: (user: User) => void; themePreference: ThemePreference; onThemeChange: (theme: ThemePreference) => void; onLogout: () => void; passwordOpen: boolean; onPasswordOpenChange: (open: boolean) => void; onOpenStaff: () => void }) {
  ...
  </ScrollView><Sheet open={profileOpen} title={user.role==='ADMIN'?'Datos del administrador':'Mis datos'} onClose={()=>setProfileOpen(false)}><View style={styles.accountCard}><AccountRow label={user.role === 'ADMIN' ? 'Administrador' : 'Recepcionista'} value={user.name}/><AccountRow label="Correo" value={user.email}/><AccountRow label="Gimnasio" value={user.gym.name}/>{user.gym.municipality&&user.gym.province?<AccountRow label="Ubicación" value={`${user.gym.municipality}, ${user.gym.province}`}/>:null}<AccountRow label="Moneda" value={user.gym.currency} last/></View></Sheet></>;
}
...
function MembersScreen({ scope, revision, initialFilter, canDelete }: ScreenProps & { initialFilter: MemberFilter; canDelete: boolean }) {
  ...
  const confirmDelete = (member: Member) => Alert.alert('Archivar miembro', `¿Deseas archivar a ${member.firstName} ${member.lastName}? Quedará inactivo y se cancelarán sus membresías vigentes. Los cobros, importes y abonos conservarán su estado y seguirán visibles en Caja.`, [{ text:'Cancelar', style:'cancel' }, { text:'Archivar', style:'destructive', onPress:() => { try { ensureActiveSubscription(); void offline.deleteMember(scope, member.id).then(() => { setSelected(null); showSuccess('Miembro archivado. Sus cobros y abonos se conservaron.'); load(); }).catch(showError); } catch(error) { showError(error); } } }]);
...
function PlansScreen({ scope, revision, readOnly }: ScreenProps & { readOnly: boolean }) {
  ...
  const deactivate = (plan: Plan) => { void onlineAction(() => Alert.alert('Desactivar plan', `¿Deseas desactivar el plan ${plan.name}? No podrá asignarse a nuevas membresías. Las membresías actuales y sus cobros no cambiarán.`, [{ text:'Cancelar', style:'cancel' }, { text:'Desactivar', style:'destructive', onPress:() => { void offline.updatePlan(scope, plan.id, { isActive:false }).then(() => syncNow(scope)).then(() => { setSelected(null); showSuccess('Plan desactivado correctamente.'); load(); }).catch(showError); } }])); };
  const reactivate = (plan: Plan) => { void onlineAction(() => Alert.alert('Reactivar plan', `¿Deseas volver a activar el plan ${plan.name}? Estará disponible para nuevas membresías.`, [{ text:'Cancelar', style:'cancel' }, { text:'Reactivar', onPress:() => { void offline.updatePlan(scope, plan.id, { isActive:true }).then(() => syncNow(scope)).then(() => { setSelected(null); showSuccess('Plan reactivado correctamente.'); load(); }).catch(showError); } }])); };
...
function PaymentsScreen({ scope, revision }: ScreenProps) {
  ...
  const debt = filteredPayments.reduce((sum, payment) => sum + Math.max(0, Number(payment.amount) - Number(payment.paidAmount)), 0);
  const pendingCount = filteredPayments.filter(payment => Number(payment.amount) > Number(payment.paidAmount)).length;
...
function PlanForm({ scope, open, plan, onClose, onSaved }: FormProps & { plan: Plan | null }) {
  ...
  const save = async () => { setSaving(true); try { ensureActiveSubscription(); if (!await hasInternetConnection()) { showOnlineRequired(); return; } const input={ name:name.trim(), price:Number(price), durationDays:Number(days), description:description.trim(), isActive }; if (plan) await offline.updatePlan(scope, plan.id, input); else await api.createPlan(input); await syncNow(scope); Keyboard.dismiss(); showSuccess(plan ? 'Plan actualizado correctamente.' : 'Plan creado correctamente.'); onSaved(); } catch(error) { showError(error); } finally { setSaving(false); } };
...
function AssignPlanForm({ scope, member, plans, onClose, onSaved }: { scope: string; member: Member | null; plans: Plan[]; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => { if (!member || !planId) return; setSaving(true); try { ensureActiveSubscription(); if (membership) await offline.updateMembership(scope, member.id, membership.id, { planId, status:'CANCELLED' }); else await offline.assignPlan(scope, { memberId: member.id, planId, periodCount, ...(Number(amount) > 0 ? { initialPayment: Number(amount), paymentMethod: 'CASH' } : {}) }); showSuccess(membership ? 'Membresía cancelada. El cobro y sus importes se conservaron.' : isRenewal ? 'Membresía renovada correctamente.' : 'Membresía asignada correctamente.'); onSaved(); } catch (error) { showError(error); } finally { setSaving(false); } };
