...
export const api = {
  ...
  createMember: (input: { ci: string; firstName: string; lastName: string; phone?: string }) => request<Member>('/members', { method: 'POST', body: JSON.stringify(input) }),
  plans: () => request<Plan[]>('/plans'),
  createPlan: (input: { name: string; price: number; durationDays: number }) => request<Plan>('/plans', { method: 'POST', body: JSON.stringify(input) }),
