    ...
    <section className="modal-card manage-card">
      ...
      {loading ? <div className="loading-block">Cargando información…</div> : <>
        ...
        {tab === "members" && <section className="manager-section">
          ...
          {(addingMember || editingMember) && <MemberEditor member={editingMember} plans={plans} currency={detail.currency ?? "CUP"} onCancel={() => { setAddingMember(false); setEditingMember(null); }} onSave={async payload => { if(editingMember) await api(`/platform/gyms/${gym.id}/members/${editingMember.id}`,token,{method:"PATCH",body:JSON.stringify(payload)}); else { const { planId, ...memberPayload } = payload; const created = await api<Member>(`/platform/gyms/${gym.id}/members`,token,{method:"POST",body:JSON.stringify(memberPayload)}); await api(`/platform/gyms/${gym.id}/members/${created.id}/memberships`,token,{method:"POST",body:JSON.stringify({ planId })}); } await complete(editingMember ? "Miembro actualizado" : "Miembro creado con su plan inicial"); }}/>}
